...
namespace {
    ...
    const std::vector<const char*> TAG_NAMES = {
...
int main(int argc, char** argv) {
    try {
        ...
        // Load the mesh file
        MB_CHK_SET_ERR(
            moab->load_file(mesh_file.c_str()),
        ...
        for (const auto& tag_name : TAG_NAMES) {
            ...
            MB_CHK_SET_ERR(
                moab->tag_get_handle(tag_name, 1, moab::MB_TYPE_INTEGER, tag_handle),
            ...
            sets.clear();
            MB_CHK_SET_ERR(
                moab->get_entities_by_type_and_tag(0, moab::MBENTITYSET, &tag_handle,
                                                 nullptr, 1, sets),
            ...
            for (const auto& set_handle : sets) {
                ...
                int set_id;
                MB_CHK_SET_ERR(
                    moab->tag_get_data(tag_handle, &set_handle, 1, &set_id),
                ...
                set_entities.clear();
                MB_CHK_SET_ERR(
                    moab->get_entities_by_handle(set_handle, set_entities, true),
                    "Failed to get entities in set"
                );

                std::cout << "  " << tag_name << " " << set_id
                          << " contains " << set_entities.size() << " entities\n";
