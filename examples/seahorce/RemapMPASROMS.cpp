...
int main( int argc, char** argv )
{
    ...
    // Load the MPAS file from disk with given options
    {
        ...
        err = ScaleCoords( mbi, mpas_verts, radius, !useTranspose, false );MB_CHK_ERR( err );
    ...
    moab::Range roms_verts, roms_elems, roms3d_verts, roms3d_elems;
    {
        ...
        // Rescale the radius of both to compute the intersection
        err = ScaleCoords( mbi, roms_verts, radius, useTranspose, false );MB_CHK_ERR( err );
    ...
    if( use_3dprojection )
    {
        if( generateExtrusions )
        {
            ...
            err =
                mbi->tag_get_handle( "refBottomDepth", src_zlayers, moab::MB_TYPE_DOUBLE, mztag, moab::MB_TAG_SPARSE );MB_CHK_SET_ERR( err, "Can't get tag handle: refBottomDepth" );
            std::vector< double > ref_zmh_z1d( src_zlayers ), zmh_xyz3d( mpas_elems.size() * src_zlayers ),
            ...
            err = ExtrudePolygonsToPolyhedra( mbi, zmh_xyz3d, mpas_covering_set, mpasset3d );MB_CHK_SET_ERR( err, "Can't extrude MPAS polygons" );
