...
class HermiteCubicCurve
{
  ...
  public:
    ...
    bool monotonic = true;
    Value edges =
        0;  // By default, enforce flat edges - you can still pass the first/last point in twice to get an angled corner
    ...
    void add( Value x, Value y )
    {
        ...
        if( !points.empty() )
        {
            ...
            if( points.size() > 1 )
            {
                ...
                if( ( prev2.lineGradient < 0 && prev.lineGradient > 0 ) ||
                    ( prev2.lineGradient > 0 && prev.lineGradient < 0 ) )
                {
                    prev.peak = true;
                }
            }
    ...
    void finish()
    {
        ...
        for( size_t i = 0; i < points.size(); ++i )
        {
            ...
            if( i > 0 && points[i - 1].step != 0 )
            {  // We have a previous point
                Point& prev = points[i - 1];
                if( i + 1 < points.size() && points[i].step != 0 )
                {  // We have a next point
                    // Different gradient modes
                    if( mode == GradientMode::quadraticX )
                    {
                        current.curveGradient =
                            ( current.lineGradient * prev.step + prev.lineGradient * current.step ) /
                    ...
                    else if( mode == GradientMode::quadraticY )
                    {
                        ...
                        {
                            current.curveGradient =
                                ( current.lineGradient * weightPrev + prev.lineGradient * weightNext ) /
                ...
                else if( i > 1 && points[i - 2].step != 0 )
                {  // We have a previous-but-one point
                    ...
                    // Don't cross 0
                    if( ( current.curveGradient > 0 && prev.lineGradient < 0 ) ||
                        ( current.curveGradient < 0 && prev.lineGradient > 0 ) )
                    {
                        current.curveGradient = 0;
                    }
                }
            ...
            else if( i + 2 < points.size() && points[i + 1].step != 0 )
            {  // We have a next-but-one point
                ...
                // Don't cross 0
                if( ( current.curveGradient > 0 && current.lineGradient < 0 ) ||
                    ( current.curveGradient < 0 && current.lineGradient > 0 ) )
                {
                    current.curveGradient = 0;
                }
            }
