...
// Function to perform monotone linear interpolation with extrapolation based on nearest-neighbor
double linearInterpolate( const Vector& x, const Vector& y, double query )
{
    int n = x.size();
    ...
    double interpolatedValue = ( 1 - t ) * y[idx] + t * y[idx + 1];
