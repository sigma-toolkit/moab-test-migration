...
// separate vertices based on coordinate y; those with y ~= 0 will be our base vertices
  ...
  for (int i=2; i<=5; i++) // first layers, 2 and 3, 4, 5
  {
  ...
  for (int i=6; i<=10; i++) // layers, 4, 5 and 6; new vertex layers 3, 4, 5, 6 
  {
    std::cout <<" Layer : " << i << endl;
    for (Range::iterator eit=orgQuads.begin(); eit!=orgQuads.end(); eit++)
    {
  ...
  rval = mb.write_file("hexes2.h5m", 0, 0, sets);MB_CHK_ERR(rval);
  return 0;
}
