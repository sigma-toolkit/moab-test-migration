...
int main( int argc, char** argv )
{
    ...
    // build box
    double box_center[ 3 ], box_axis1[ 3 ], box_axis2[ 3 ], box_axis3[ 3 ], pnt_start[ 3 ],
        ray_length;
    ...
    rval = tool.ray_intersect_triangles( intersections, intersection_facets, tree_root, 10e-12,
                                         pnt_start, box_axis1, &ray_length );
    if( rval != moab::MB_SUCCESS )
    {
    ...
    std::cout << "ray start point: " << pnt_start[ 0 ] << " " << pnt_start[ 1 ] << " "
              << pnt_start[ 2 ] << std::endl;
    std::cout << " ray direction: " << box_axis1[ 0 ] << " " << box_axis1[ 1 ] << " "
              << box_axis1[ 2 ] << "\n";
