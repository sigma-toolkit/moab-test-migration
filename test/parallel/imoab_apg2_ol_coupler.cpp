 ...
 *  similar for land, but land will be intersected too with the atm
...
// load phys atm mesh, with some data on it already
  ...
  if (lndComm != MPI_COMM_NULL) {
    ...
    // load the next component mesh; land is now similar to ocean
    PUSH_TIMER("Load LND mesh")
    ierr = iMOAB_LoadMesh(cmpLndPID, lndFilename.c_str(), readopts.c_str(), &nghlay, lndFilename.length(), readopts.length() );
    CHECKIERR(ierr, "Cannot load LND mesh on cmpLndPID")
  ...
  if (atmCouComm != MPI_COMM_NULL)
  {
    ...
    ierr = iMOAB_ComputeCommGraph(cmpPhAtmPID, cplAtmLndPID, &atmCouComm, &atmPEGroup, &couPEGroup,
...
//#endif
  ...
  if (couComm != MPI_COMM_NULL) {
    ...
    ierr = iMOAB_ComputeScalarProjectionWeights ( cplAtmOcnPID, weights_identifiers[0],
                                                  ...
                                                  strlen(disc_methods[1]), strlen(disc_methods[1]),
  ...
  if (couComm != MPI_COMM_NULL) {
    ...
    ierr = iMOAB_ReceiveElementTag(cplAtmOcnPID, "T_ph;u_ph;v_ph;", &atmCouComm, &cmpatm, strlen("T_ph;u_ph;v_ph;"));
...
// we used this to compute
  ...
  if (couComm != MPI_COMM_NULL){
    ...
    ierr = iMOAB_ReceiveElementTag(cplAtmLndPID, "T_ph;u_ph;v_ph;", &atmCouComm, &cmpatm, strlen("T_ph;u_ph;v_ph;"));
  ...
  if (couComm != MPI_COMM_NULL) {
    ...
    const char * concat_fieldnameT = "T_proj;u_proj;v_proj;";
      ...
      /* We have the remapping weights now. Let us apply the weights onto the tag we defined
         on the source mesh and get the projection on the target mesh */
    PUSH_TIMER("Apply Scalar projection weights")
    ierr = iMOAB_ApplyScalarProjectionWeights ( cplAtmLndPID, weights_identifiers[0],
                                              concat_fieldname,
                                              concat_fieldnameT,
                                              strlen(weights_identifiers[0]),
                                              strlen(concat_fieldname),
