...
int main( int argc, char* argv[] )
{
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        ierr = iMOAB_ComputeScalarProjectionWeights(
            ...
            &fValidate, &sparseConstraints, dof_tag_names[0].c_str(), dof_tag_names[1].c_str(),
            weights_identifiers[0].size(), disc_methods[0].size(), disc_methods[1].size(), dof_tag_names[0].size(),
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        ierr = iMOAB_ComputeScalarProjectionWeights(
            ...
            &fValidate, &sparseConstraints, dof_tag_names[0].c_str(), dof_tag_names[2].c_str(),
            weights_identifiers[1].size(), disc_methods[0].size(), disc_methods[2].size(), dof_tag_names[0].size(),
