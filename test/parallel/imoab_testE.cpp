...
int main( int argc, char* argv[] )
{
...
#ifdef ENABLE_OCNATM_COUPLING
        ierr =
            iMOAB_RegisterApplication( "ATM2X", &couComm, &cplatm2, cplAtm2PID );  // atm on coupler pes, ocean context
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        ierr = iMOAB_ComputeMeshIntersectionOnSphere(
            cplOcnPID, cplAtm2PID,
            cplOcnAtm2PID );  // coverage mesh was computed here, for cplAtmPID, atm on coupler pes
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        ierr = iMOAB_ComputeScalarProjectionWeights( cplOcnAtm2PID, weights_identifiers[1], disc_methods[1],
                                                     &disc_orders[1],                   // fv
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        ierr =
            iMOAB_DefineTagStorage( cplAtmPID, bottomTempProjectedField3, &tagTypes[1], &atmCompNDoFs, &tagIndex[1] );
        CHECKIERR( ierr, "failed to define the field tag T_proj3" );
        ierr =
            iMOAB_DefineTagStorage( cplAtmPID, bottomUVelProjectedField3, &tagTypes[1], &atmCompNDoFs, &tagIndex[1] );
        CHECKIERR( ierr, "failed to define the field tag u_proj3" );
        ierr =
            iMOAB_DefineTagStorage( cplAtmPID, bottomVVelProjectedField3, &tagTypes[1], &atmCompNDoFs, &tagIndex[1] );
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        CHECKIERR( ierr, "failed to define the field tag T_proj" );
        ierr =
            iMOAB_DefineTagStorage( cplAtm2PID, bottomTempProjectedField, &tagTypes[1], &atmCompNDoFs, &tagIndex[1] );
        ...
        CHECKIERR( ierr, "failed to define the field tag u_proj" );
        ierr =
            iMOAB_DefineTagStorage( cplAtm2PID, bottomUVelProjectedField, &tagTypes[1], &atmCompNDoFs, &tagIndex[1] );
        ...
        CHECKIERR( ierr, "failed to define the field tag v_proj" );
        ierr =
            iMOAB_DefineTagStorage( cplAtm2PID, bottomVVelProjectedField, &tagTypes[1], &atmCompNDoFs, &tagIndex[1] );
