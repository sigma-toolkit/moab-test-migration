/*
 * zoltan_repartition_test.cpp
 *
 *  zoltan_repartition_test will contain tests for migrating meshes in parallel environments, with iMOAB
...
ErrorCode migrate_smart( const char* filename, const char* outfile, int partMethod, int startG1 = 0, int endG1 = 0, int startG2 = 0, int endG2 = 1 )
{
    ...
    if (rank == startG2)
      std::cout << "\n\nComputing mesh repartition from (" << startG1 << ", " << endG1 << ") -> (" << startG2 << ", " << endG2 << ")\n"; 
    ...
    if( comm2 != MPI_COMM_NULL )
    {
        ...
        int nvv[3], nve[3], ldata[2];
        int lsize = endG2-startG2+1;
        std::vector<int> gdata;
        if (rank == groupTasks[0]) gdata.resize(2*lsize);
        
        // compute the number of elements in each part
        ierr = iMOAB_GetMeshInfo(pid2, nvv, nve, nullptr, nullptr, nullptr);
        CHECKRC( ierr, "cannot get mesh info" )
        ldata[0] = nvv[0]; ldata[1] = nve[0];

        // std::cout << lsize << ", " <<  rank << ": vertices =" << nvv[0] << ", elements = " << nve[0] << std::endl; 
        //MPI_Barrier(jcomm);
        MPI_Gather(ldata, 2, MPI_INT, gdata.data(), 2, MPI_INT, groupTasks[0], jcomm);
        if (rank == groupTasks[0]) {
          
          std::cout << "\t vertices = [ "; 
          for ( int ll=0; ll < lsize; ++ll )
            std::cout << gdata[ll*2] << ", ";
          std::cout << " ]\n";

          std::cout << "\t elements = [ "; 
          for ( int ll=0; ll < lsize; ++ll )
            std::cout << gdata[ll*2+1] << ", ";
          std::cout << " ]\n";

          std::cout.flush();
        }

        // std::cout << rank << ": vertices =" << nvv[0] << ", elements = " << nve[0] << std::endl; 
...
ErrorCode migrate_combinations( const char* filename )
{
  ErrorCode rval;
  const int partition_method = 2; // 0: trivial, 1: graph, 2: geom
  rval = migrate_smart( filename, "migrate_geom_1_16.h5m", partition_method, 0, 0, 0, 15 );MB_CHK_ERR(rval);
  rval = migrate_smart( filename, "migrate_geom_2_16.h5m", partition_method, 0, 1, 0, 15 );MB_CHK_ERR(rval);
  rval = migrate_smart( filename, "migrate_geom_3_16.h5m", partition_method, 0, 2, 0, 15 );MB_CHK_ERR(rval);
  rval = migrate_smart( filename, "migrate_geom_4_16.h5m", partition_method, 0, 3, 0, 15 );MB_CHK_ERR(rval);
  rval = migrate_smart( filename, "migrate_geom_7_16.h5m", partition_method, 0, 6, 0, 15 );MB_CHK_ERR(rval);
  rval = migrate_smart( filename, "migrate_geom_11_16.h5m", partition_method, 0, 10, 0, 15 );MB_CHK_ERR(rval);
  rval = migrate_smart( filename, "migrate_geom_13_16.h5m", partition_method, 0, 12, 0, 15 );MB_CHK_ERR(rval);
...
int main( int argc, char* argv[] )
{
    ...
    int num_errors = RUN_TEST_ARG2( migrate_combinations, filename.c_str() );
