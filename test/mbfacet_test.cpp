...
ErrorCode gentityset_test( FBEngine* pFacet )
{
    ...
    for( ; ent_type < num_type; ent_type++ )
    {
        ...
        // 1 means isList, ordered
        MB_CHK_SET_ERR( pFacet->createEntSet( 1, &ges_array[ent_type] ), "Problem creating entityset." );
    ...
    MB_CHK_SET_ERR( pFacet->createEntSet( 1, &super_set ), "Failed to create a super set in gentityset_test." );
