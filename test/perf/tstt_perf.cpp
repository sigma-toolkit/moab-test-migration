...
#define HANDLE_ARRAY_PTR( array ) \
    reinterpret_cast< Entity_Handle* >( array._get_ior( )->d_firstElement )
...
void testB( TSTTM::Mesh& mesh, const int nelem, sidl::array< double >& coords, const int* connect )
{
    ...
    try
    {
        mesh_arrmod.createVtxArr( num_verts, TSTTM::StorageOrder_BLOCKED, coords, 3 * num_verts,
    ...
    try
    {
        mesh_arrmod.createEntArr( TSTTM::EntityTopology_HEXAHEDRON, sidl_connect, sidl_connect_size,
                                  new_hexes, new_hexes_size, status, status_size );
    }
    catch( TSTT::Error& /* err */ )
    {
...
#define VINDEX( i, j, k ) ( i + ( j * numv ) + ( k * numv_sq ) )
    ...
    for( int i = 0; i < num_verts; i++ )
    {
        ...
        try
        {
            mesh_arrmod.createVtxArr( 1, TSTTM::StorageOrder_BLOCKED, tmp_coords, tmp_coords_size,
    ...
    for( int i = 0; i < nelem; i++ )
    {
        for( int j = 0; j < nelem; j++ )
        {
            for( int k = 0; k < nelem; k++ )
            {
                ...
                try
                {
                    mesh_arrmod.createEntArr( TSTTM::EntityTopology_HEXAHEDRON, tmp_conn,
                                              tmp_conn_size, new_hex, new_hex_size, status,
                                              status_size );
                }
                catch( TSTT::Error& /* err */ )
                {
...
void query_elem_to_vert( TSTTM::Mesh& mesh )
{
    ...
    try
    {
        mesh.getEntities( 0, TSTTM::EntityType_REGION, TSTTM::EntityTopology_HEXAHEDRON, all_hexes,
                          all_hexes_size );
    }
    catch( TSTT::Error& /* err */ )
    {
    ...
    try
    {
        ...
        for( int i = 0; i < all_hexes_size; i++ )
        {
            ...
            mesh_ent.getEntAdj( all_hexes_ptr[ i ], TSTTM::EntityType_VERTEX, dum_connect,
                                dum_connect_size );
            ...
            mesh.getVtxArrCoords( dum_connect, dum_connect_size, order, dum_coords,
                                  dum_coords_size );
...
void query_vert_to_elem( TSTTM::Mesh& mesh )
{
    ...
    try
    {
        mesh.getEntities( 0, TSTTM::EntityType_VERTEX, TSTTM::EntityTopology_POINT, all_verts,
                          all_verts_size );
    }
    catch( TSTT::Error& /* err */ )
    {
    ...
    try
    {
        ...
        for( int i = 0; i < all_verts_size; i++ )
        {
            ...
            mesh_ent.getEntAdj( all_verts_ptr[ i ], TSTTM::EntityType_REGION, dum_hexes,
                                dum_hexes_size );
        }
    }
    catch( TSTT::Error& /* err */ )
...
#define VINDEX( i, j, k ) ( i + ( j * numv ) + ( k * numv_sq ) )
    ...
    coords_ptr[ VINDEX( 0, nelem, 0 ) ] = coords_ptr[ VINDEX( 0, nelem, 0 ) + 2 * ( nelem + 1 ) ] =
        0.0;
    ...
    coords_ptr[ VINDEX( 0, nelem, nelem ) + nelem + 1 ] =
        coords_ptr[ VINDEX( 0, nelem, nelem ) + 2 * ( nelem + 1 ) ] = LENGTH;
    coords_ptr[ VINDEX( nelem, 0, 0 ) ] = LENGTH;
    coords_ptr[ VINDEX( nelem, 0, 0 ) + nelem + 1 ] =
        coords_ptr[ VINDEX( nelem, 0, 0 ) + 2 * ( nelem + 1 ) ] = 0.0;
    coords_ptr[ VINDEX( nelem, 0, nelem ) ] =
        coords_ptr[ VINDEX( nelem, 0, nelem ) + 2 * ( nelem + 1 ) ] = LENGTH;
    coords_ptr[ VINDEX( nelem, 0, nelem ) + nelem + 1 ] = 0.0;
    coords_ptr[ VINDEX( nelem, nelem, 0 ) ] = coords_ptr[ VINDEX( nelem, nelem, 0 ) + nelem + 1 ] =
        LENGTH;
    coords_ptr[ VINDEX( nelem, nelem, 0 ) + 2 * ( nelem + 1 ) ] = 0.0;
    coords_ptr[ VINDEX( nelem, nelem, nelem ) ] =
        coords_ptr[ VINDEX( nelem, nelem, nelem ) + nelem + 1 ] =
