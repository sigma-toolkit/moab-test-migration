...
int main( int argc, char* argv[] )
{
    ...
    for( size_t i = 0; i < verts.size(); ++i )
        if( fixed[i] ) skin.push_back( verts[i] );
    ...
    while( !next.empty() )
    {
        ...
        while( !curr.empty() )
        {
            ...
            for( size_t i = 0; i < conn.size(); ++i )
            {
                ...
                if( d_iter != depth.end() && d_iter->second < min ) min = d_iter->second;
            }
    ...
    for( size_t i = 0; i < skin.size(); ++i )
    {
        if( !std::binary_search( higher_order.begin(), higher_order.end(), skin[i] ) ) continue;
