...
def test_pymoab_core_base_path_warning(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules["pymoab.paths"].__dict__, '__path__', ["/mock/source/pymoab"])

    with mock.patch("os.path.exists", side_effect=[False, True, True]), \
         mock.patch("sysconfig.get_path", return_value="/mock/source"), \
         pytest.warns(RuntimeWarning, match="It seems that PyMOAB is being run from its source directory"):
        importlib.reload(paths)
