...
int main( int argc, char* argv[] )
{
    ...
    bool centers_only = false;

    opts.addOpt< void >( "center_only,c", "project centers only", &centers_only );
    string extraTag;
    opts.addOpt< std::string >( "tag,t", "extra tag", &extraTag );
    ...
    {
        rval = mb->tag_get_handle( extraTag.c_str(), etag );MB_CHK_ERR( rval );
    }
    ...
    rval = IntxUtils::global_gnomonic_projection( mb, sf, R, centers_only, outSet, etag );MB_CHK_ERR( rval );
