...
int main( int argc, char* argv[] )
{
    ...
    int filter = 0, fMonotoneTypeID = 0, fVolumetric = 0, fValidate = 0, fNoConserve = 0, fNoBubble = 1, fInverseDistanceMap = 0;
    ...
    ierr = iMOAB_ApplyScalarProjectionWeights( atmocnPID, &filter, weights_identifiers[0].c_str(), bottomTempField.c_str(),
                                               bottomTempProjectedNCField.c_str() );
    CHECKIERR( ierr, "failed to apply projection weights for scalar non-conservative field" );

    /* We have the remapping weights now. Let us apply the weights onto the tag we defined
       on the srouce mesh and get the projection on the target mesh */
    ierr = iMOAB_ApplyScalarProjectionWeights( atmocnPID, &filter, weights_identifiers[2].c_str(), bottomTempField.c_str(),
    ...
    /* We have the remapping weights now. Let us apply the weights onto the tag we defined
       on the srouce mesh and get the projection on the target mesh */
    ierr = iMOAB_ApplyScalarProjectionWeights( atmlndPID, &filter, weights_identifiers[1].c_str(), bottomTempField.c_str(),
                                               bottomTempProjectedField.c_str() );
    CHECKIERR( ierr, "failed to apply projection weights for ATM-LND scalar field" );

    /* We have the remapping weights now. Let us apply the weights onto the tag we defined
       on the srouce mesh and get the projection on the target mesh */
    ierr = iMOAB_ApplyScalarProjectionWeights( lndatmPID, &filter, weights_identifiers[1].c_str(),
    ...
    ierr = iMOAB_ApplyScalarProjectionWeights( lndatmPID, &filter, weights_identifiers[3].c_str(),
                                               bottomTempProjectedField.c_str(), bottomTempFieldATM.c_str() );
