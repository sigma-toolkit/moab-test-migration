...
int main( int argc, char* argv[] )
{
    ...
    if( ( dims.size() >= 1 && dims.size() <= 2 ) && ( vertex_data || cell_data || edge_data ) )
    {
        ...
        int def_val = 0;
        MB_CHK_SET_ERR( mb->tag_get_handle( variable_name.c_str(), (int)size_tag, mbtype, newTag,
    ...
    if( !sefile_name.empty() )
    {
        ...
        Tag newTag2;
        MB_CHK_SET_ERR( mb2->tag_get_handle( variable_name.c_str(), (int)sizeTag, mbtype, newTag2,
                                             MB_TAG_CREAT | MB_TAG_DENSE, &dfield[0] ),
                        "can't define new tag" );
