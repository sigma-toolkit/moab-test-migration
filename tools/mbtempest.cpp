...
struct ToolContext
{
    ...
    void timer_pop( )
    {
        ...
        // std::cout << "\n[LOG" << proc_id << "] Time taken to " << opName << " = " << timer->time_since_birth() -
        // timer_ops << std::endl;
        opName.clear( );
    }
    ...
    void ParseCLOptions( int argc, char* argv[] )
    {
        ...
        opts.addOpt<void>( "advfront,a", "Use the advancing front intersection instead of the Kd-tree based algorithm "
    ...
    if( runCtx->meshType == moab::TempestRemapper::OVERLAP_MEMORY )
    {
...

        {
            ...
            double initial_sarea = areaAdaptor.area_on_sphere(
                mbCore, runCtx->meshsets[ 0 ], radius_src );  // use the target to compute the initial area
            double initial_tarea = areaAdaptor.area_on_sphere(
                mbCore, runCtx->meshsets[ 1 ], radius_dest );  // use the target to compute the initial area
        ...
        if( runCtx->computeWeights )
        {
            ...
            int err = GenerateOfflineMapWithMeshes(
                ...
                runCtx
                    ->ensureMonotonicity  // bool fNoBubble = false, bool fCorrectAreas = false, int fMonotoneTypeID = 0
            );
            runCtx->timer_pop( );
    ...
    else if( runCtx->meshType == moab::TempestRemapper::OVERLAP_MOAB )
    {
        ...
        // print some diagnostic checks to see if the overlap grid resolved the input meshes correctly
        {
            ...
            if( !proc_id )
            {
                outputFormatter.printf(
                    0, "initial area: source mesh = %12.14f, target mesh = %12.14f, overlap mesh = %12.14f\n",
                    global_areas[ 0 ], global_areas[ 1 ], global_areas[ 2 ] );
                // outputFormatter.printf ( 0, " area with l'Huiller: %12.14f with Girard: %12.14f\n", global_areas[2],
                // global_areas[3] ); outputFormatter.printf ( 0, " relative difference areas = %12.10e\n", fabs (
                // global_areas[2] - global_areas[3] ) / global_areas[2] );
        ...
        if( runCtx->computeWeights )
        {
            ...
            rval = weightMap->GenerateRemappingWeights(
                ...
                runCtx->fNoCheck,  // bool fVolumetric=false, bool fNoConservation=false, bool fNoCheck=false,
                runCtx->doftag_names[ 0 ],
                runCtx->doftag_names[ 1 ],  // std::string source_tag_name, std::string target_tag_name,
                "",  //"",                                                       // std::string strVariables="",
                "", "",  // std::string strInputData="", std::string strOutputData="",
                "", true,  // std::string strNColName="", bool fOutputDouble=true,
                "", false,
                0.0,  // std::string strPreserveVariables="", bool fPreserveAll=false, double dFillValueOverride=0.0,
            ...
            if( runCtx->outFilename.size( ) )
            {
                ...
                if( proc_id == 0 )
                {
                    ...
                    metafile << "Generator = MOAB-TempestRemap (mbtempest) Offline Regridding Weight Generator"
                             << std::endl;
            ...
            if( runCtx->verifyWeights )
            {
                ...
                runCtx->timer_pop( );
                // rval = mbCore->write_file ( "srcWithSolnTag.h5m", NULL, writeOptions, &runCtx->meshsets[0], 1 );
                // MB_CHK_ERR ( rval );
                ...
                // rval = mbCore->write_file ( "tgtWithSolnTag.h5m", NULL, writeOptions, &runCtx->meshsets[1], 1 );
                // MB_CHK_ERR ( rval );
                runCtx->timer_pop( );
...
static moab::ErrorCode CreateTempestMesh( ToolContext& ctx, moab::TempestRemapper& remapper, Mesh* tempest_mesh )
{
    ...
    {
        // For the overlap method, choose between: "fuzzy", "exact" or "mixed"
        err = GenerateOverlapMesh( ctx.inFilenames[ 0 ], ctx.inFilenames[ 1 ], *tempest_mesh, ctx.outFilename,
                                   "NetCDF4", "exact", true );
