...
MsqIMeshP::MsqIMeshP( iMesh_Instance mesh, iMeshP_PartitionHandle partition,
                      iBase_EntitySetHandle meshset, iBase_EntityType type, MsqError& err,
...
void MsqIMeshP::vertices_get_global_id( const VertexHandle vert_array[], size_t gid[],
                                        size_t num_vtx, MsqError& /*err*/ )
{
    ...
    int                parts_size = 0;
    iMeshP_getLocalParts( meshInstance, partitionInstance, &parts, &parts_allocated, &parts_size,
                          &itaps_err );
    iMeshP_Part part_id;
    iMeshP_getPartIdFromPartHandle( meshInstance, partitionInstance, parts[ 0 ], &part_id,
                                    &itaps_err );
    ...
    {
        iMeshP_getEntOwnerPart( meshInstance, partitionInstance,
                                ( iBase_EntityHandle )( vert_array[ i ] ), &part_id, &itaps_err );
        int part_rank;
        ...
        else
        {
            ...
            iMeshP_getOwnerCopy( meshInstance, partitionInstance,
                                 ( iBase_EntityHandle )( vert_array[ i ] ), &part_id, &handle,
                                 &itaps_err );
            gid[ i ] = (size_t)handle;
        }
    }
...
void MsqIMeshP::vertices_get_processor_id( const VertexHandle vert_array[], int pid[],
                                           size_t num_vtx, MsqError& /*err*/ )
{
    int itaps_err;
    for( unsigned i = 0; i < num_vtx; i++ )
