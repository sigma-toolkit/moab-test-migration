...
class MsqMOAB : virtual public MBMesquite::Mesh
{
  public:
     ...
     *                  position.
     */
    MsqMOAB( moab::Core* imesh, moab::EntityHandle meshset, moab::EntityType element_dimension,
             MsqError& err, const moab::Tag* fixed_tag = 0, const moab::Tag* slaved_tag = 0 );
     ...
     *                  position.
     */
    MsqMOAB( moab::Core* imesh, moab::EntityType element_dimension, MsqError& err,
             const moab::Tag* fixed_tag = 0, const moab::Tag* slaved_tag = 0 );
     ...
     *                  is to be improved.
     */
    void set_active_set( moab::EntityHandle meshset, moab::EntityType element_dimension,
                         MsqError& err );
    ...
    /**\brief Get vertex mark */
    virtual void vertices_get_byte( const VertexHandle* vert_array, unsigned char* byte_array,
                                    size_t array_size, MsqError& err );
  ...
  protected:
     ...
     */
    void get_adjacent_entities( const moab::EntityHandle* source, size_t num_source,
                                moab::EntityType target_type, std::vector< EntityHandle >& target,
                                std::vector< size_t >& offsets, MsqError& err );
  ...
  private:
    void init_active_mesh( moab::Core* mesh, MsqError& err, const moab::Tag* fixed_tag,
                           const moab::Tag* slaved_tag );

    void get_flag_data( moab::Tag tag, bool have_tag, moab::DataType type,
                        const VertexHandle vert_array[], std::vector< bool >& flag_array,
                        size_t num_vtx, MsqError& err );

    /** \brief Set tag values */
    void tag_set_data( TagHandle handle, size_t num_elems, const EntityHandle* handle_array,
                       const void* tag_data, MsqError& err );

    /** \brief Get tag values */
    void tag_get_data( TagHandle handle, size_t num_elems, const EntityHandle* handle_array,
