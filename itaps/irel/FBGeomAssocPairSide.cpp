...
FBGeomAssocPairSide::FBGeomAssocPairSide( iRel_Instance p_relation, iBase_Instance p_instance,
                                          int p_id )
    : relation( p_relation ), instance_( reinterpret_cast< FBiGeom_Instance >( p_instance ) ),
      id( p_id )
{
    int result;
...
int FBGeomAssocPairSide::create_relation_side( )
{
    ...
    if( result == iBase_TAG_NOT_FOUND )
    {
        FBiGeom_createTag( instance_, rel_tag_name.c_str( ), 1, iBase_ENTITY_HANDLE, &relation_tag,
...
int FBGeomAssocPairSide::get_all_entities( int dimension, iBase_EntityHandle** entities,
                                           int* entities_alloc, int* entities_size )
{
    int this_type = ( dimension == -1 ? iBase_ALL_TYPES : dimension );
    int result;

    FBiGeom_getEntities( instance_, 0, this_type, entities, entities_alloc, entities_size,
                         &result );
    PROCESS_ERROR;
    RETURNR( iBase_SUCCESS );
}

int FBGeomAssocPairSide::get_all_sets( iBase_EntitySetHandle** sets, int* sets_alloc,
                                       int* sets_size )
{
    int result;
...
int FBGeomAssocPairSide::get_entities( int dimension, iBase_EntitySetHandle set_handle,
                                       ...
                                       int* entities_size )
{
    ...
    FBiGeom_getEntities( instance_, set_handle, this_type, entities, entities_alloc, entities_size,
                         &result );
    PROCESS_ERROR;
    RETURNR( iBase_SUCCESS );
}

int FBGeomAssocPairSide::get_ents_dims( iBase_EntityHandle* entities, int entities_size,
                                        int** ent_types, int* ent_types_alloc, int* ent_types_size )
{
    int result;

    FBiGeom_getArrType( instance_, entities, entities_size, ent_types, ent_types_alloc,
...
int FBGeomAssocPairSide::get_related_sets( iBase_EntitySetHandle** /*sets*/, int* /*sets_alloc*/,
                                           int* /*sets_size*/ )
{
    RETURNR( iBase_FAILURE );
}

int FBGeomAssocPairSide::get_relation_side( iBase_EntityHandle* entities, int num_entities,
                                            void* values )
{
    ...
    FBiGeom_getArrData( instance_, entities, num_entities, relation_tag, &values, &values_alloc,
                        &values_size, &result );
    PROCESS_ERROR;
    RETURNR( iBase_SUCCESS );
}

int FBGeomAssocPairSide::get_relation_side( iBase_EntitySetHandle* sets, int num_sets,
                                            void* values )
{
...
int FBGeomAssocPairSide::set_relation_side( iBase_EntityHandle* entities, int num_entities,
                                            const void* values )
{
    int result;
...
int FBGeomAssocPairSide::set_relation_side( iBase_EntitySetHandle* sets, int num_sets,
                                            const void* values )
{
...
int FBGeomAssocPairSide::get_gids( iBase_EntityHandle* entities, int num_entities, int* values )
{
    ...
    FBiGeom_getArrData( instance_, entities, num_entities, gid_tag,
                        reinterpret_cast< void** >( &values ), &values_alloc, &values_size,
                        &result );
    PROCESS_ERROR;
    RETURNR( iBase_SUCCESS );
}
...
int FBGeomAssocPairSide::get_gids( iBase_EntitySetHandle* sets, int num_sets, int* values )
{
    ...
    for( int i = 0; i < num_sets; i++ )
    {
        FBiGeom_getEntSetData( instance_, sets[ i ], gid_tag, reinterpret_cast< void** >( &data ),
...
int FBGeomAssocPairSide::get_dims( iBase_EntityHandle* /*entities*/, int /*num_entities*/,
                                   int* /*values*/ )
{
    RETURNR( iBase_FAILURE );
}
