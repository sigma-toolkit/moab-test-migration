...
double Intx2MeshInPlane::setup_tgt_cell(EntityHandle tgt, int & nsTgt)
{
  ...
  // get coordinates of the tgt quad
  double cellArea =0;
  int num_nodes;
...
ErrorCode Intx2MeshInPlane::computeIntersectionBetweenTgtAndSrc(EntityHandle tgt,
    EntityHandle src, double * P, int & nP, double & area, int markb[MAXEDGES],
  ...
  if (check_boxes_first) {
    ...
    if (!GeomUtil::bounding_boxes_overlap(tgtCoords, nsTgt, srcCoords, nsSrc,
        box_error))
  ...
  if (extraPoints >= 1) {
    for (int k = 0; k < nsSrc; k++) {
      if (side[k]) {
        // this means that vertex k of src is inside convex tgt; mark edges k-1 and k in src,
        ...
        markb[k] = 1;
        markb[(k + nsSrc - 1) % nsSrc] = 1; // it is the previous edge, actually, but instead of doing -1, it is
  ...
  for (int i = 0; i < nP; i++)
  {
    ...
    if (!found)
    {
      ...
      for (j = 0; j < nsTgt; j++)
      {
        ...
        if (dbg_1)
          std::cout << "   edge " << j << ": "
              << mb->id_from_handle(adjTgtEdges[j]) << " " << tgtConn[j] << " "
