                                                 ...
                                                 Tag etag )
{
    ...
    double vals[16];  // just to store the tag value, if needed
    if( !partSets.empty() )
    {
    ...
    if( centers_only )
    {
        for( Range::iterator it = inputRange.begin(); it != inputRange.end(); ++it )
        {
            ...
            if( etag )
            {
                rval = mb->tag_get_data( etag, &cell, 1, vals );MB_CHK_ERR( rval );
                rval = mb->tag_set_data( etag, &vertex, 1, vals );MB_CHK_ERR( rval );
            }
        }
    }
    else
    {
        ...
        Range subranges[6];
        std::vector< std::set< EntityHandle > > subsets;
        subsets.resize( 6 );
        for( Range::iterator it = inputRange.begin(); it != inputRange.end(); ++it )
        {
            CartVect center;
            ...
            int plane;
            decide_gnomonic_plane( center, plane );
            subsets[plane - 1].insert( cell );
        }
        for( int i = 1; i <= 6; i++ )
        {
            std::copy( subsets[i - 1].rbegin(), subsets[i - 1].rend(), range_inserter( subranges[i - 1] ) );
            Range verts;
            ...
            for( Range::iterator eit = subranges[i - 1].begin(); eit != subranges[i - 1].end(); ++eit )
            {
                ...
                if( etag )
                {
                    rval = mb->tag_get_data( etag, &eh, 1, vals );MB_CHK_ERR( rval );
                    rval = mb->tag_set_data( etag, &newCell, 1, vals );MB_CHK_ERR( rval );
                }
            }
        }
    }
