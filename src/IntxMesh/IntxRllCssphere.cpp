                                                                ...
                                                                int& nsTgt, bool check_boxes_first )
{
    ...
    extraPoints = IntxUtils::borderPointsOfCSinRLL(
        tgtCoords, tgtCoords2D, nsTgt, srcCoords, nsSrc, srcEdgeType, &( P[ 2 * nP ] ), side,
        100 * epsilon_area );  // we need to compare with 0 a volume from 3 vector product; // lots
                               // of round off errors at stake
    if( extraPoints >= 1 )
    {
