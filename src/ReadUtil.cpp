...
ErrorCode ReadUtil::get_node_coords( const int /*num_arrays*/, const int num_nodes,
                                     const int               preferred_start_id,
                                     EntityHandle&           actual_start_handle,
                                     std::vector< double* >& arrays, int sequence_size )
{
    ErrorCode       error;
    ...
    error = mMB->sequence_manager( )->create_entity_sequence(
        MBVERTEX, num_nodes, 0, preferred_start_id, actual_start_handle, seq, sequence_size );
    ...
    error = static_cast< VertexSequence* >( seq )->get_coordinate_arrays( arrays[ 0 ], arrays[ 1 ],
                                                                          arrays[ 2 ] );
...
ErrorCode ReadUtil::get_element_connect( const int num_elements, const int verts_per_element,
                                        ...
                                        EntityHandle& start_handle )
{
    ...
    ErrorCode       error;
    EntitySequence* seq;
    error = mMB->sequence_manager( )->create_meshset_sequence( num_sets, start_id, flags,
                                                               start_handle, seq );
    if( MB_SUCCESS != error ) return error;
                                        ...
                                        const EntityHandle* conn_array )
{
    ...
    if( adj_fact != NULL && adj_fact->vert_elem_adjacencies( ) )
    {
        ...
        {
            adj_fact->notify_create_entity( tmp_hndl, ( conn_array + j ),
                                            number_vertices_per_element );
            tmp_hndl++;
...
ErrorCode ReadUtil::gather_related_ents( Range& partition, Range& related_ents,
                                         EntityHandle* file_set )
{
    ...
    for( int dim = 3; dim >= 0; dim-- )
    {
        ...
        ErrorCode tmp_result =
            mMB->get_adjacencies( related_ents, dim, false, tmp_ents, Interface::UNION );
...
ErrorCode ReadUtil::get_ordered_vertices( EntityHandle* bound_ents, int* sense, int bound_size,
                                          int dim, EntityHandle* bound_verts, EntityType& etype )
{
    ...
    for( EntityType t = MBEDGE; t < MBENTITYSET; t++ )
    {
        ...
        {
            result = mMB->get_connectivity( bound_ents[ index ], connect, num_connect, false,
                                            &tmp_connect );
            if( MB_SUCCESS != result ) return result;
            ...
            for( int c = 0; c < num_connect; c++ )
            {
                ...
                {
                    bound_verts[ indices[ c ] ] =
                        ( sense[ index ] > 0 ) ? connect[ c ] : connect[ num_connect - c - 1 ];
                    numv++;
                }
            }
            if( numv == nverts )
...
ErrorCode ReadUtil::create_gather_set( EntityHandle& gather_set )
{
    ...
    Tag gather_set_tag;
    rval = mMB->tag_get_handle( "GATHER_SET", 1, MB_TYPE_INTEGER, gather_set_tag,
...
ErrorCode ReadUtil::get_gather_set( EntityHandle& gather_set )
{
    Tag       gather_set_tag;
    ErrorCode rval =
        mMB->tag_get_handle( "GATHER_SET", 1, MB_TYPE_INTEGER, gather_set_tag, MB_TAG_SPARSE );
    if( MB_SUCCESS != rval ) return rval;
    ...
    Range gather_sets;
    rval =
        mMB->get_entities_by_type_and_tag( 0, MBENTITYSET, &gather_set_tag, vals, 1, gather_sets );
    if( MB_SUCCESS != rval ) return rval;
