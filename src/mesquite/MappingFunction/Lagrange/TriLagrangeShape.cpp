...
void TriLagrangeShape::coefficients( Sample loc, NodeSet nodeset, double* coeff_out,
                                     size_t* indices_out, size_t& num_coeff, MsqError& err ) const
{
    if( nodeset.have_any_mid_face_node( ) )
    {
        MSQ_SETERR( err )
        ( "TriLagrangeShape does not support mid-element nodes", MsqError::UNSUPPORTED_ELEMENT );
        return;
    }
...
static inline void get_linear_derivatives( size_t* vertices, MsqVector< 2 >* derivs )
{
    vertices[ 0 ] = 0;
    vertices[ 1 ] = 1;
    vertices[ 2 ] = 2;
                                     ...
                                     MsqVector< 2 >* derivs, size_t& num_vtx )
{
    ...
    double corner_derivs[ 3 ][ 2 ] = { { -1.0, -1.0 }, { 1.0, 0.0 }, { 0.0, 1.0 } };
...
void TriLagrangeShape::ideal( Sample, MsqMatrix< 3, 2 >& J, MsqError& ) const
{
