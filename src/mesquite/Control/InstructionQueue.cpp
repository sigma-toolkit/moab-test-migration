...
void InstructionQueue::add_vertex_slaver( VertexSlaver* vs, MsqError& )
{
    ...
    if( isMasterSet ) ++masterInstrIndex;
    ++vertexSlaverCount;
...
/*! \fn InstructionQueue::add_preconditioner(QualityImprover* instr, MsqError &err)
  ...
  */
void InstructionQueue::add_preconditioner( QualityImprover* instr, MsqError& err )
{
    if( isMasterSet )
    {
        MSQ_SETERR( err )
...
*/
void InstructionQueue::insert_preconditioner( QualityImprover* instr, size_t index, MsqError& err )
{
    // checks index is valid
...
*/
void InstructionQueue::add_quality_assessor( QualityAssessor* instr, MsqError& /*err*/ )
{
    instructions.push_back( instr );
}
...
/*! \fn InstructionQueue::insert_quality_assessor(QualityAssessor* instr, size_t index, MsqError
   &err) \brief inserts a QualityAssessor* into the instruction queue.
...
void InstructionQueue::set_master_quality_improver( QualityImprover* instr, MsqError& err )
{
    if( isMasterSet )
    {
