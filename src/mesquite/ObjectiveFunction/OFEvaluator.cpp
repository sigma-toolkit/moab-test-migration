...
bool OFEvaluator::initialize( MeshDomainAssoc* mesh_and_domain, const Settings* settings,
                              PatchSet* user_set, MsqError& err )
{
    ...
    bool result = get_objective_function( )->initialize_block_coordinate_descent(
        mesh_and_domain, settings, user_set, err );
    return !MSQ_CHKERR( err ) && result;
}

void OFEvaluator::initialize_queue( MeshDomainAssoc* mesh_and_domain, const Settings* settings,
                                    MsqError& err )
{
    if( get_objective_function( ) )
        get_objective_function( )->initialize_queue( mesh_and_domain, settings, err );MSQ_ERRRTN( err );
}
...
bool OFEvaluator::update( PatchData& pd, double& value, MsqError& err )
{
    ...
    bool b =
        get_objective_function( )->evaluate( currUpdateType, pd, value, OF_FREE_EVALS_ONLY, err );
                          ...
                          MsqError& err )
{
    ...
    bool b =
        get_objective_function( )->evaluate_with_gradient( currUpdateType, pd, value, grad, err );
    currUpdateType = updateType;
    return !MSQ_CHKERR( err ) && b;
}
                          ...
                          std::vector< SymMatrix3D >& hess_diag, MsqError& err )
{
    ...
    bool b = get_objective_function( )->evaluate_with_Hessian_diagonal( currUpdateType, pd, value,
                                                                        grad, hess_diag, err );
    currUpdateType = updateType;
    return !MSQ_CHKERR( err ) && b;
}
                          ...
                          MsqHessian& Hessian, MsqError& err )
{
    ...
    bool b = get_objective_function( )->evaluate_with_Hessian( currUpdateType, pd, value, grad,
                                                               Hessian, err );
    currUpdateType = updateType;
    return !MSQ_CHKERR( err ) && b;
}
                            ...
                            std::vector< SymMatrix3D >& hess_diag, MsqError& err ) const
{
    ...
    bool b = get_objective_function( )->evaluate_with_Hessian_diagonal( tempType, pd, value, grad,
                                                                        hess_diag, err );
    return !MSQ_CHKERR( err ) && b;
}

bool OFEvaluator::evaluate( PatchData& pd, double& value, std::vector< Vector3D >& grad,
                            MsqHessian& Hessian, MsqError& err ) const
{
    if( !have_objective_function( ) )
    {
        MSQ_SETERR( err )( "No ObjectiveFunction", MsqError::INVALID_STATE );
    ...
    bool b =
        get_objective_function( )->evaluate_with_Hessian( tempType, pd, value, grad, Hessian, err );
    return !MSQ_CHKERR( err ) && b;
}
