...
int ParallelHelperImpl::comm_smoothed_vtx_tnb( MsqError& err )
{
    ...
    for( i = 0; i < (long)neighbourProc.size(); i++ )
    {
        if( numVtxPerProcRecv[i] )
        {
            ...
            rval =
                MPI_Irecv( packed_vertices_import[i], 4 * numVtxPerProcRecv[i], MPI_DOUBLE_PRECISION, neighbourProc[i],
        ...
        if( numVtxPerProcSend[i] )
        {
            ...
            rval =
                MPI_Isend( packed_vertices_export[i], 4 * numVtxPerProcSend[i], MPI_DOUBLE_PRECISION, neighbourProc[i],
...
int ParallelHelperImpl::comm_smoothed_vtx_nb( MsqError& err )
{
    ...
    for( j = 0; j < (long)neighbourProc.size(); j++ )
    {
        ...
        {
            rval =
                MPI_Isend( packed_vertices_export[j], 4 * numVtxPerProcSend[j], MPI_DOUBLE_PRECISION, neighbourProc[j],
...
/// function correct, if all proc's values are true, it returns true.  @see communicate_any_true
/// above

/// return true in @param value if all procs have value=true, else if any have it false, return
/// false
