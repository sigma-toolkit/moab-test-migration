...
class ParallelComm
{
  public:
    ...
    ParallelComm( Interface* impl, std::vector< unsigned char >& tmp_buff, MPI_Comm comm,
                  int* pcomm_id_out = 0 );
    ...
    static ParallelComm* get_pcomm( Interface* impl, EntityHandle partitioning,
    ...
    //! assign a global id space, for largest-dimension or all entities (and
    //! in either case for vertices too)
    ErrorCode assign_global_ids( Range entities[], const int dimension, const int start_id,
    ...
    ErrorCode send_entities( const int to_proc, Range& orig_ents, const bool adjs, const bool tags,
                             const bool store_remote_handles, const bool is_iface,
                             Range& final_ents, int& incoming1,
    ...
    ErrorCode send_entities( std::vector< unsigned int >& send_procs,
                             std::vector< Range* >& send_ents, int& incoming1, int& incoming2,
    ...
    ErrorCode recv_entities( const int from_proc, const bool store_remote_handles,
                             const bool is_iface, Range& final_ents, int& incomming1,
                             int& incoming2, std::vector< std::vector< EntityHandle > >& L1hloc,
    ...
    ErrorCode recv_messages( const int from_proc, const bool store_remote_handles,
                             const bool is_iface, Range& final_ents, int& incoming1, int& incoming2,
    ...
    ErrorCode recv_remote_handle_messages( const int from_proc, int& incoming2,
                                           std::vector< EntityHandle >& L2hloc,
    ...
    ErrorCode exchange_ghost_cells( int ghost_dim, int bridge_dim, int num_layers, int addl_ents,
                                    bool store_remote_handles, bool wait_all = true,
     ...
     */
    static ErrorCode exchange_ghost_cells( ParallelComm** pc, unsigned int num_procs, int ghost_dim,
                                           int bridge_dim, int num_layers, int addl_ents,
    ...
    ErrorCode exchange_owned_mesh( std::vector< unsigned int >& exchange_procs,
                                   ...
                                   std::vector< MPI_Request >&  recv_ent_reqs,
                                   std::vector< MPI_Request >&  recv_remoteh_reqs,
                                   const bool recv_posted, bool store_remote_handles, bool wait_all,
                                   bool migrate = false );
     ...
     *                 entities
     */
    ErrorCode reduce_tags( const std::vector< Tag >& src_tags, const std::vector< Tag >& dst_tags,
                           const MPI_Op mpi_op, const Range& entities );
     ...
     */
    ErrorCode broadcast_entities( const int from_proc, Range& entities,
                                  const bool adjacencies = false, const bool tags = true );
     ...
     * \param tags If true, all non-default-valued tags are sent for sent entities
     */
    ErrorCode scatter_entities( const int from_proc, std::vector< Range >& entities,
                                const bool adjacencies = false, const bool tags = true );
    ...
    ErrorCode get_remote_handles( EntityHandle* local_vec, EntityHandle* rem_vec, int num_ents,
                                  int to_proc );
     ...
     */
    ErrorCode resolve_shared_ents( EntityHandle this_set, Range& proc_ents, int resolve_dim = -1,
                                   int shared_dim = -1, Range* skin_ents = NULL,
                                   const Tag* id_tag = 0 );
    ...
    static ErrorCode resolve_shared_ents( ParallelComm** pc, const unsigned int np,
                                          EntityHandle this_set, const int to_dim );
     ...
     */
    ErrorCode get_sharing_data( const EntityHandle entity, int* ps, EntityHandle* hs,
     ...
     */
    ErrorCode get_sharing_data( const EntityHandle* entities, int num_entities,
                                std::set< int >& procs, int op = Interface::INTERSECT );

    /** \brief Get the intersection or union of all sharing processors
     * Same as previous variant but with range as input
     */
    ErrorCode get_sharing_data( const Range& entities, std::set< int >& procs,
     ...
     */
    ErrorCode get_shared_entities( int other_proc, Range& shared_ents, int dim = -1,
                                   const bool iface = false, const bool owned_filter = false );
    /*
    ...
    */
    ...
    ErrorCode get_shared_proc_tags( Tag& sharedp_tag, Tag& sharedps_tag, Tag& sharedh_tag,
                                    Tag& sharedhs_tag, Tag& pstatus_tag );
    ...
    ErrorCode get_owning_part( EntityHandle entity, int& owning_part_id_out,
                               EntityHandle* owning_handle = 0 );
    ErrorCode get_sharing_parts( EntityHandle entity, int part_ids_out[ MAX_SHARING_PROCS ],
     ...
     *       pointed-to range
     */
    ErrorCode filter_pstatus( Range& ents, const unsigned char pstatus_val, const unsigned char op,
                              int to_proc = -1, Range* returned_ents = NULL );
    ...
    ErrorCode pack_buffer( Range& orig_ents, const bool adjacencies, const bool tags,
                           const bool store_remote_handles, const int to_proc, Buffer* buff,
                           TupleList* entprocs = NULL, Range* allsent = NULL );

    ErrorCode unpack_buffer( unsigned char* buff_ptr, const bool store_remote_handles,
                             const int from_proc, const int ind,
                             std::vector< std::vector< EntityHandle > >& L1hloc,
                             std::vector< std::vector< EntityHandle > >& L1hrem,
                             std::vector< std::vector< int > >&          L1p,
                             std::vector< EntityHandle >&                L2hloc,
    ...
    ErrorCode pack_entities( Range& entities, Buffer* buff, const bool store_remote_handles,
                             const int to_proc, const bool is_iface, TupleList* entprocs = NULL,
                             Range* allsent = NULL );

    //! unpack entities in buff_ptr
    ErrorCode unpack_entities(
    ...
    ErrorCode pack_remote_handles( std::vector< EntityHandle >& L1hloc,
                                   std::vector< EntityHandle >& L1hrem, std::vector< int >& procs,
                                   unsigned int to_proc, Buffer* buff );
    ...
    ErrorCode create_interface_sets(
    ...
    ErrorCode
        tag_shared_verts( TupleList&                                                   shared_ents,
                          std::map< std::vector< int >, std::vector< EntityHandle > >& proc_nvecs,
     ...
     */
    ErrorCode gather_data( Range& gather_ents, Tag& tag_handle, Tag id_tag = 0,
                           EntityHandle gather_set = 0, int root_proc_rank = 0 );
     ...
     */
    ErrorCode
        settle_intersection_points( Range& edges, Range& shared_edges_owned,
  ...
  private:
    ErrorCode reduce_void( int tag_data_type, const MPI_Op mpi_op, int num_ents, void* old_vals,
                           void* new_vals );
    ...
    void print_debug_irecv( int to, int from, unsigned char* buff, int size, int tag,
                            int incoming );
    ...
    ErrorCode set_sharing_data( EntityHandle ent, unsigned char pstatus, int old_nump, int new_nump,
    ...
    ErrorCode get_sent_ents( const bool is_iface, const int bridge_dim, const int ghost_dim,
                             const int num_layers, const int addl_ents, Range* sent_ents,
                             Range& allsent, TupleList& entprocs );
     ...
     */
    ErrorCode set_pstatus_entities( Range& pstatus_ents, unsigned char pstatus_val,
     ...
     *        existing value is over-written
     */
    ErrorCode set_pstatus_entities( EntityHandle* pstatus_ents, int num_ents,
                                    unsigned char pstatus_val, bool lower_dim_ents = false,
    ...
    ErrorCode send_buffer( const unsigned int to_proc, Buffer* send_buff, const int msg_tag,
                           MPI_Request& send_req, MPI_Request& ack_recv_req, int* ack_buff,
                           int& this_incoming, int next_mesg_tag = -1,
    ...
    ErrorCode recv_buffer( int mesg_tag_expected, const MPI_Status& mpi_status, Buffer* recv_buff,
                           MPI_Request& recv_2nd_req, MPI_Request& ack_req, int& this_incoming,
                           Buffer* send_buff, MPI_Request& send_req, MPI_Request& sent_ack_req,
                           bool& done, Buffer* next_buff = NULL, int next_tag = -1,
                           MPI_Request* next_req = NULL, int* next_incoming = NULL );

    //! pack a range of entities with equal # verts per entity, along with
    //! the range on the sending proc
    ErrorCode pack_entity_seq( const int nodes_per_entity, const bool store_remote_handles,
                               const int to_proc, Range& these_ents,
                               std::vector< EntityHandle >& entities, Buffer* buff );
    ...
    ErrorCode pack_sets( Range& entities, Buffer* buff, const bool store_handles,
                         const int to_proc );

    ErrorCode unpack_sets( unsigned char*& buff_ptr, std::vector< EntityHandle >& entities,
                           const bool store_handles, const int to_proc );

    ErrorCode pack_adjacencies( Range& entities, Range::const_iterator& start_rit,
                                Range& whole_range, unsigned char*& buff_ptr, int& count,
                                const bool just_count, const bool store_handles,
                                const int to_proc );

    ErrorCode unpack_adjacencies( unsigned char*& buff_ptr, Range& entities,
    ...
    ErrorCode find_existing_entity( const bool is_iface, const int owner_p,
                                    const EntityHandle owner_h, const int num_ents,
                                    const EntityHandle* connect, const int num_connect,
                                    const EntityType this_type, std::vector< EntityHandle >& L2hloc,
                                    std::vector< EntityHandle >& L2hrem,
                                    std::vector< unsigned int >& L2p, EntityHandle& new_h );

    ErrorCode build_sharedhps_list( const EntityHandle entity, const unsigned char pstatus,
                                    const int sharedp, const std::set< unsigned int >& procs,
     ...
     */
    ErrorCode pack_tags( Range& entities, const std::vector< Tag >& src_tags,
                         const std::vector< Tag >& dst_tags, const std::vector< Range >& tag_ranges,
                         Buffer* buff, const bool store_handles, const int to_proc );
    ...
    ErrorCode pack_tag( Tag source_tag, Tag destination_tag, const Range& entities,
                        const std::vector< EntityHandle >& whole_range, Buffer* buff,
                        const bool store_remote_handles, const int to_proc );

    ErrorCode unpack_tags( unsigned char*& buff_ptr, std::vector< EntityHandle >& entities,
                           const bool store_handles, const int to_proc,
                           const MPI_Op* const mpi_op = NULL );

    ErrorCode
        tag_shared_verts( TupleList& shared_verts, Range* skin_ents,
                          std::map< std::vector< int >, std::vector< EntityHandle > >& proc_nvecs,
                          Range&                                                       proc_verts );

    ErrorCode
        get_proc_nvecs( int resolve_dim, int shared_dim, Range* skin_ents,
                        std::map< std::vector< int >, std::vector< EntityHandle > >& proc_nvecs );
    ...
    ErrorCode update_iface_sets( Range& sent_ents, std::vector< EntityHandle >& remote_handles,
                                 int from_proc );
    ...
    ErrorCode get_ghosted_entities( int bridge_dim, int ghost_dim, int to_proc, int num_layers,
                                    int addl_ents, Range& ghosted_ents );
    ...
    //! debugging.
    ErrorCode
        exchange_all_shared_handles( std::vector< std::vector< SharedEntityData > >& send_data,
    ...
    ErrorCode get_remote_handles( const bool store_remote_handles, EntityHandle* from_vec,
    ...
    //! different here
    ErrorCode get_remote_handles( const bool store_remote_handles, const Range& from_range,
                                  Range& to_range, int to_proc,
                                  const std::vector< EntityHandle >& new_ents );
    ...
    ErrorCode get_local_handles( EntityHandle* from_vec, int num_ents,
                                 const std::vector< EntityHandle >& new_ents );

    ErrorCode update_remote_data( Range& local_range, Range& remote_range, int other_proc,
                                  const unsigned char add_pstat );

    ErrorCode update_remote_data( const EntityHandle new_h, const int* ps, const EntityHandle* hs,
                                  const int num_ps, const unsigned char add_pstat );

    ErrorCode update_remote_data_old( const EntityHandle new_h, const int* ps,
...
inline ErrorCode ParallelComm::get_shared_proc_tags( Tag& sharedp, Tag& sharedps, Tag& sharedh,
                                                     Tag& sharedhs, Tag& pstatus )
{
    sharedp = sharedp_tag( );
...
inline ErrorCode ParallelComm::reduce_tags( const char* tag_name, const MPI_Op mpi_op,
                                            const Range& entities )
{
    // get the tag handle
    std::vector< Tag > tags( 1 );
...
inline ErrorCode ParallelComm::get_sharing_data( const EntityHandle* entities, int num_entities,
                                                 std::set< int >& procs, int op )
{
    Range dum_range;
...
inline ErrorCode ParallelComm::get_sharing_data( const EntityHandle entity, int* ps,
                                                 EntityHandle* hs, unsigned char& pstat,
                                                 int& num_ps )
{
    unsigned int dum_ps;
