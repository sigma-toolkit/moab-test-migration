...
ErrorCode ParallelData::get_partition_sets( Range& part_sets, const char* tag_name )
{
    ...
    if( NULL != tag_name )
        ...
        result =
            mbImpl->tag_get_handle( PARALLEL_PARTITION_TAG_NAME, 1, MB_TYPE_INTEGER, part_tag );
    ...
    result = mbImpl->get_entities_by_type_and_tag( 0, MBENTITYSET, &part_tag, NULL, 1, part_sets,
                                                   Interface::UNION );
    return result;
}
...
#define CONTINUE             \
    ...
    for( int i = 0; i < 2; i++ )
    {
        ...
        if( 0 == i )
            tmp_result = mbImpl->tag_get_handle( PARALLEL_SHARED_PROC_TAG_NAME, 1, MB_TYPE_INTEGER,
                                                 proc_tag );
        else
            tmp_result = mbImpl->tag_get_handle( PARALLEL_SHARED_PROCS_TAG_NAME, MAX_SHARING_PROCS,
                                                 MB_TYPE_INTEGER, proc_tag );
        ...
        Range proc_sets;
        tmp_result = mbImpl->get_entities_by_type_and_tag( 0, MBENTITYSET, &proc_tag, NULL, 1,
        ...
        for( k = 0, rit = proc_sets.begin( ); rit != proc_sets.end( ); ++rit, k++ )
        {
            for( int j = 0; j < tsize; j++ )
            {
                if( my_rank != proc_tags[ 2 * k + j ] && proc_tags[ 2 * k + j ] >= 0 )
                    iface_data.insert(
                        std::pair< int, EntityHandle >( proc_tags[ 2 * k + j ], *rit ) );
            }
        }
    }
