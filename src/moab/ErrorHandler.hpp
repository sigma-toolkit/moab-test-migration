...
#define MB_SET_ERR( err_code, err_msg )                                                  \
    ...
    {                                                                                    \
        ...
        return moab::MBError( __LINE__, __func__, __FILENAME__, __MBSDIR__, err_code,    \
                              err_ostr.str( ).c_str( ), moab::MB_ERROR_TYPE_NEW_LOCAL ); \
    } while( false )
...
#define MB_SET_ERR_RET( err_msg )                                                      \
    ...
    {                                                                                  \
        std::ostringstream err_ostr;                                                   \
        err_ostr << err_msg;                                                           \
        moab::MBError( __LINE__, __func__, __FILENAME__, __MBSDIR__, moab::MB_FAILURE, \
                       err_ostr.str( ).c_str( ), moab::MB_ERROR_TYPE_NEW_LOCAL );      \
        return;                                                                        \
    } while( false )
...
#define MB_SET_GLB_ERR( err_code, err_msg )                                               \
    ...
    {                                                                                     \
        ...
        err_ostr << err_msg;                                                              \
        return moab::MBError( __LINE__, __func__, __FILENAME__, __MBSDIR__, err_code,     \
                              err_ostr.str( ).c_str( ), moab::MB_ERROR_TYPE_NEW_GLOBAL ); \
    } while( false )
...
#define MB_CHK_ERR_RET( err_code )                                                     \
    ...
    {                                                                                  \
        if( moab::MB_SUCCESS != err_code )                                             \
        {                                                                              \
            moab::MBError( __LINE__, __func__, __FILENAME__, __MBSDIR__, err_code, "", \
                           moab::MB_ERROR_TYPE_EXISTING );                             \
            return;                                                                    \
        }                                                                              \
    } while( false )
