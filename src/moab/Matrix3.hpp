...
namespace Matrix
{
    ...
    template< typename Matrix > inline Matrix mmult3( const Matrix& a, const Matrix& b )
    {
    ...
    template< typename Matrix > inline const Matrix inverse( const Matrix& d )
    {
...
#define DEPRECATED
    ...
    template< typename Vector > inline ErrorCode eigen_decomposition( Vector& evals, Matrix3& evecs )
    {
        ...
        if( bisSymmetric )
        {
            ...
            const Eigen::SelfAdjointEigenSolver< Eigen::Matrix3d >::RealVectorType& e3evals =
    ...
    template< typename Vector > inline void copycol( int index, Vector& vol )
    {
#ifndef MOAB_HAVE_LAPACK
    ...
    template< typename Vector > inline Vector vcol( int index ) const
    {
...
#else
        return ( _mat[ 0 ] * _mat[ 4 ] * _mat[ 8 ] + _mat[ 1 ] * _mat[ 5 ] * _mat[ 6 ] +
                 _mat[ 2 ] * _mat[ 3 ] * _mat[ 7 ] - _mat[ 0 ] * _mat[ 5 ] * _mat[ 7 ] -
                 _mat[ 1 ] * _mat[ 3 ] * _mat[ 8 ] - _mat[ 2 ] * _mat[ 4 ] * _mat[ 6 ] );
#endif
    }
