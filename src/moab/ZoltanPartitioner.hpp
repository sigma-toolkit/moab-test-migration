...
void mbGetObjectList( void* userDefinedData, int numGlobalIds, int numLids, ZOLTAN_ID_PTR gids,
                      ZOLTAN_ID_PTR lids, int wgt_dim, float* obj_wgts, int* err );
...
void mbGetNumberOfEdges( void* userDefinedData, int numGlobalIds, int numLids, int numObjs,
                         ZOLTAN_ID_PTR gids, ZOLTAN_ID_PTR lids, int* numEdges, int* err );

void mbGetEdgeList( void* userDefinedData, int numGlobalIds, int numLids, int numObjs,
...
class ZoltanPartitioner : public PartitionerBase< int >
{
  ...
  public:
    ZoltanPartitioner( Interface* impl = NULL, const bool use_coords = false, int argc = 0,
                       char** argv = NULL
#ifdef MOAB_HAVE_CGM
                       ,
    ...
    ErrorCode balance_mesh( const char* zmethod, const char* other_method,
    ...
    virtual ErrorCode partition_mesh_and_geometry(
        ...
        const bool spherical_coords = false, const bool print_time = false );
    ...
    virtual ErrorCode write_partition( const int nparts, Range& elems, const int* assignment,
    ...
    ErrorCode repartition( std::vector< double >& x, std::vector< double >& y,
                           std::vector< double >& z, int StartID, const char* zmethod,
                           Range& localGIDs );
    ...
    ErrorCode partition_owned_cells( Range& owned, ParallelComm* pco,
                                     std::multimap< int, int >& extraGraphEdges,
                                     std::map< int, int > procs, int& numNewPartitions,
                                     std::map< int, Range >& distribution, int met );

#ifdef MOAB_HAVE_CGM
    ErrorCode write_partition( const int nparts, DLIList< RefEntity* > entities,
                               const int* assignment, std::vector< double >& obj_weights,
                               const bool part_surf, const bool ghost );

    ErrorCode partition_surface( const int nparts, DLIList< RefEntity* > entities,
                                 const int* assignment, std::vector< double >& obj_weights );
#endif
  ...
  private:
    ...
    // moab_ids
    ErrorCode assemble_graph( const int dimension, std::vector< double >& coords,
                              std::vector< int >& moab_ids, std::vector< int >& adjacencies,
                              std::vector< int >& length, Range& elems, bool part_geom = false,
    ...
    ErrorCode assemble_graph( const int dimension, std::vector< double >& coords,
                              std::vector< int >& moab_ids, std::vector< int >& adjacencies,
                              std::vector< int >& length, std::vector< double >& obj_weights,
                              std::vector< double >& edge_weights, std::vector< int >& parts,
                              DLIList< RefEntity* >& entities, const double part_geom_mesh_size,
                              const int n_part );

    ErrorCode partition_round_robin( const int n_part );

    ErrorCode partition_child_entities( const int dim, const int n_part, const bool partition_surf,
    ...
    void mbFinalizePoints( int npts, int numExport, ZOLTAN_ID_PTR exportLocalIDs, int* exportProcs,
                           int** assignment );

    int mbInitializePoints( int npts, double* pts, int* ids, int* adjs, int* length,
                            double* obj_weights = NULL, double* edge_weights = NULL,
...
inline ErrorCode ZoltanPartitioner::partition_mesh( const int nparts, const char* method,
                                                    const int part_dim, const bool write_as_sets,
                                                    const bool write_as_tags, const bool,
                                                    const bool, const char*, const bool print_time )
{
    return partition_mesh_and_geometry( -1.0, nparts, method, NULL, 1.03, part_dim, write_as_sets,
                                        write_as_tags, 0, 0, false, false, false, print_time );
}
