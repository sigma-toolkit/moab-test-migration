...
void mbGetObjectList( void* userDefinedData, int numGlobalIds, int numLids, ZOLTAN_ID_PTR gids, ZOLTAN_ID_PTR lids,
                      int wgt_dim, float* obj_wgts, int* err );

int mbGetObjectSize( void* userDefinedData, int* err );

void mbGetObject( void* userDefinedData, int numGlobalIds, int numLids, int numObjs, ZOLTAN_ID_PTR gids,
                  ZOLTAN_ID_PTR lids, int numDim, double* pts, int* err );

void mbGetNumberOfEdges( void* userDefinedData, int numGlobalIds, int numLids, int numObjs, ZOLTAN_ID_PTR gids,
                         ZOLTAN_ID_PTR lids, int* numEdges, int* err );

void mbGetEdgeList( void* userDefinedData, int numGlobalIds, int numLids, int numObjs, ZOLTAN_ID_PTR gids,
                    ZOLTAN_ID_PTR lids, int* numEdges, ZOLTAN_ID_PTR nborGlobalIds, int* nborProcs, int wgt_dim,
                    float* edge_wgts, int* err );

void mbGetPart( void* userDefinedData, int numGlobalIds, int numLids, int numObjs, ZOLTAN_ID_PTR gids,
                ZOLTAN_ID_PTR lids, int* part, int* err );
...
class ZoltanPartitioner : public PartitionerBase< int >
{
  ...
  public:
...
#ifdef MOAB_HAVE_MPI
    ErrorCode balance_mesh( EntityHandle initialSet, const char* zmethod, const char* other_method, int num_parts,
            std::map<int, Range> &distributes, const bool write_as_sets = true,
                            const bool write_as_tags = false , int dimension = 3 );
#endif
    ...
    virtual ErrorCode write_partition( const int nparts, Range& elems, const int* assignment, const bool write_as_sets,
    ...
    ErrorCode write_partition( const int nparts, DLIList< RefEntity* > entities, const int* assignment,
                               std::vector< double >& obj_weights, const bool part_surf, const bool ghost );

    ErrorCode partition_surface( const int nparts, DLIList< RefEntity* > entities, const int* assignment,
  ...
  private:
    ...
    ErrorCode assemble_graph( const int dimension, std::vector< double >& coords, std::vector< int >& moab_ids,
                              std::vector< int >& adjacencies, std::vector< int >& length,
                              std::vector< double >& obj_weights, std::vector< double >& edge_weights,
                              std::vector< int >& parts, DLIList< RefEntity* >& entities,
    ...
    int mbInitializePoints( int npts, double* pts, int* ids, int* adjs, int* length, double* obj_weights = NULL,
                            double* edge_weights = NULL, int* parts = NULL, bool part_geom = false );
...
inline ErrorCode ZoltanPartitioner::partition_mesh( const int nparts, const char* method, const int part_dim,
                                                    const bool write_as_sets, const bool write_as_tags, const bool,
