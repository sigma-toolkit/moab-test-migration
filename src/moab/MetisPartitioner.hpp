...
class MetisPartitioner : public PartitionerBase< idx_t >
{
  ...
  public:
    ...
    virtual ErrorCode partition_mesh_and_geometry(
        const double part_geom_mesh_size, const idx_t nparts, const char* zmethod,
    ...
    virtual ErrorCode partition_mesh( const idx_t nparts, const char* method,
...
inline ErrorCode MetisPartitioner::partition_mesh_and_geometry(
    const double, const idx_t nparts, const char* zmethod, const char*, double, const int part_dim,
    const bool write_as_sets, const bool write_as_tags, const int, const int, const bool,
    const bool, const bool, const bool print_time )
{
    // Only partition the mesh - no geometric partition available
    return partition_mesh( nparts, zmethod, part_dim, write_as_sets, write_as_tags, false, false,
                           NULL, print_time );
}
