...
namespace element_utility
{
    ...
    template< typename Entity_handle, typename Matrix > class Linear_tet_map
    {
      private:
      ...
      private:
        template< typename Point > bool is_contained( const Point& result, const double tol = 1e-6 )
        {
            double sum = 0.0;
        ...
        template< typename Points > void set_tet( const Entity_handle _eh, const Points& v )
        {
            if( eh != _eh )
            {
                eh = _eh;
