...
namespace common_tree
{
    ...
    template< typename T, typename Stream > void print_vector( const T& v, Stream& out )
    {
...
#ifdef TREE_DEBUG
    template< typename T > void print_vector( const T& begin, const T& end )
    {
        std::cout << "[ ";
    ...
    namespace
    {
        template< typename T > struct Compute_center : public std::binary_function< T, T, T >
        {
    ...
    template< typename T = float > class Box
    {
      public:
    ...
    template< typename T > std::ostream& operator<<( std::ostream& out, const Box< T >& box )
    {
        out << "Max: ";
    ...
    template< typename T1, typename T2 > struct _Element_data
    {
    ...
    template< typename Box > void update_bounding_box( Box& a, const Box& b )
    {
