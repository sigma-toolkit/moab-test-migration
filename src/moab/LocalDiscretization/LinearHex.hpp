...
/**\brief Shape function space for trilinear hexahedron, obtained by a pushforward of the canonical
...
class LinearHex
{
  public:
    ...
    static ErrorCode normalFcn( const int ientDim, const int facet, const int nverts,
                                const double* verts, double normal[] );
    ...
    {
        return EvalSet( evalFcn, reverseEvalFcn, normalFcn, jacobianFcn, integrateFcn,
                        (InitFcn)NULL, insideFcn );
    }
