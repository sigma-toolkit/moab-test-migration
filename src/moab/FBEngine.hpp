...
class FBEngine
{
  public:
    ...
    ErrorCode getEntBoundBox( EntityHandle this_gent, double* x0, double* y0, double* z0,
                              double* x1, double* y1, double* z1 );
    ErrorCode getEntClosestPt( EntityHandle this_gent, double x, double y, double z, double* x1,
                               double* y1, double* y3 );
    ...
    ErrorCode getPntRayIntsct( double x, double y, double z, double dir_x, double dir_y,
                               double dir_z, std::vector< EntityHandle >& intersect_entity_handles,
                               /* int storage_order,*/
    ...
    ErrorCode createTag( const char* tag_name, int tag_num_type_values, int tag_type,
                         Tag& tag_handle_out );
    ...
    ErrorCode getArrData( const moab::EntityHandle* entity_handles, int entity_handles_size,
                          Tag tag_handle, void* tag_values_out );

    ErrorCode setArrData( const EntityHandle* entity_handles, int entity_handles_size,
    ...
    ErrorCode getEgEvalXYZ( EntityHandle edge, double x, double y, double z, double& on_x,
                            double& on_y, double& on_z, double& tngt_i, double& tngt_j,
                            double& tngt_k, double& cvtr_i, double& cvtr_j, double& cvtr_k );
    ErrorCode getFcEvalXYZ( EntityHandle face, double x, double y, double z, double& on_x,
    ...
    ErrorCode split_surface_with_direction( EntityHandle face, std::vector< double >& xyz,
    ...
    ErrorCode split_edge_at_mesh_node( EntityHandle edge, EntityHandle node,
                                       EntityHandle& new_edge );

    ErrorCode split_bedge_at_new_mesh_node( EntityHandle b_edge, EntityHandle atNode,
    ...
    ErrorCode create_volume_with_direction( EntityHandle newFace1, EntityHandle newFace2,
    ...
    ErrorCode weave_lateral_face_from_edges( EntityHandle bEdge, EntityHandle tEdge,
  ...
  private:
    ...
    ErrorCode compute_intersection_points( EntityHandle& face, EntityHandle from, EntityHandle to,
    ...
    ErrorCode separate( EntityHandle face, std::vector< EntityHandle >& chainedEdges, Range& first,
                        Range& second );
    ...
    ErrorCode boundary_nodes_on_face( EntityHandle                 face,
    ...
    ErrorCode chain_able_edge( EntityHandle edge, double min_dot, EntityHandle& next_edge,
                               bool& chainable );
    ...
    std::map< EntityHandle, EntityHandle >
        _brokenEdges;  // this is a map between splitting nodes and edges that
