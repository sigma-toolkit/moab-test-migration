...
/**
  \brief Load the projection weights from disk to transfer a solution from a source surface mesh to a destination mesh
  defined on a sphere. The intersection of the mesh should be computed a-priori.
  ...
  remap_weights_filename  (iMOAB_String)       The filename path to the mapping file to load in memory. \param[in]
  owned_dof_ids   (int *)                      The list of DoF global IDs that the current process owns and expects back
  from the root process \param[in] owned_dof_ids_length   (int *)               The length of the owned_dof_ids
  parameter array \param[in] row_major_ownership   (int *)                The flag to indicate whether specified global
  IDs correspond to source (0) or target grid (1) \param[in] remap_weights_filename_length   (int)        The length of
  the mapping file name string \param[in] solution_weights_identifier_length   (int)   The length of the solution
  weights identifier string
*/
...
/**
  \brief Write the projection weights to disk in order to transfer a solution from a source surface mesh to a
  destination mesh defined on a sphere.
  ...
  weights locally. Typically, values could be identifiers such as "scalar", "flux" or "custom". \param[in]
  remap_weights_filename  (iMOAB_String)       The filename path to the mapping file to load in memory. \param[in]
  remap_weights_filename_length   (int)        The length of the mapping file name string \param[in]
  solution_weights_identifier_length   (int)   The length of the solution weights identifier string
*/
...
/**
  ...
  discretization order for the solution field on the source grid \param[in] fNoBubble           (int *) The flag to
  indicate whether to use bubbles in the interior of SE nodes. Default: true i.e., no bubbles used \param[in]
  fMonotoneTypeID     (int *)                   The flag to indicate whether solution monotonicity is to be preserved.
  0: none, 1: \param[in] fVolumetric         (int *)                   The flag to indicate whether we need to compute
  volumetric projection weights \param[in] fNoConservation     (int *)                   The flag to indicate whether to
  ignore conservation of solution field during projection \param[in] fValidate           (int *)                   The
  flag to indicate whether to validate the consistency and conservation of solution field during projection; Production
  runs should not have this flag enabled to minimize collective operations. \param[in] source_solution_tag_dof_name
  ...
  degrees-of-freedom for the target discretization \param[in] solution_weights_identifier_length   (int)    The length
  of the solution weights identifier string \param[in] disc_method_src_length   (int)                The length of the
  discretization type string on the source mesh \param[in] disc_method_tgt_length   (int)                The length of
  the discretization type string on the target mesh \param[in] source_solution_tag_dof_name_length   (int)   The length
  of the source solution DoF tag name string \param[in] target_solution_tag_dof_name_length   (int)   The length of the
  target solution DoF tag name string
*/
