 ...
 * int x = 0;
 * po.addOpt<int>( ",x", "Specify the x number", &x ); // x will be automatically set when options
 * parsed po.parseCommandLine( argc, argv ); bool verbose = po.numOptSet("verbose") > 0; std::string
 * foo; if( !po.getOpt( "foo", &foo ) ) foo = "default";
 * ...
 *
