...
class TempestOnlineMap : public OfflineMap
{
  ...
  public:
    ...
    moab::ErrorCode GenerateRemappingWeights(
        std::string strInputType = "fv", std::string strOutputType = "fv", const int nPin = 1,
        const int nPout = 1, bool fBubble = false, int fMonotoneTypeID = 0,
        ...
        const std::string strPreserveVariables = "", const bool fPreserveAll = false,
        const double dFillValueOverride = 0.0, const bool fInputConcave = false,
  ...
  public:
    ...
    ///     if (transpose)  \p tgtVals = [A(T->S)]^T * \srcVals
    ///	</summary>
    moab::ErrorCode ApplyWeights( moab::Tag srcSolutionTag, moab::Tag tgtSolutionTag,
                                  bool transpose = false );
    ...
    ///     error norms.
    /// </summary>
    moab::ErrorCode ComputeMetrics( Remapper::IntersectionContext ctx, moab::Tag& exactTag,
