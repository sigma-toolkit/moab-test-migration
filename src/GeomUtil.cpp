...
#define CHECK_RANGE( A, B, R )                              \
    ...
    bool box_linear_elem_overlap( const CartVect* elem_corners, EntityType type, const CartVect& dims )
    {
        ...
        for( f = 0; f < num_face; ++f )
        {
            ...
            {
                case MBTRI:
                    norm = tri_norm( elem_corners[ indices[ 0 ] ], elem_corners[ indices[ 1 ] ],
                                     elem_corners[ indices[ 2 ] ] );
                    break;
                case MBQUAD:
                    norm = quad_norm( elem_corners[ indices[ 0 ] ], elem_corners[ indices[ 1 ] ],
                                      elem_corners[ indices[ 2 ] ], elem_corners[ indices[ 3 ] ] );
                    break;
                default:
                    assert( false );
                    continue;
    ...
    bool box_hex_overlap( const CartVect* elem_corners, const CartVect& center, const CartVect& dims )
    {
        ...
        double         dot, cross[ 2 ], tmp;
        CartVect       norm;
        const CartVect corners[ 8 ] = { elem_corners[ 0 ] - center, elem_corners[ 1 ] - center,
                                        elem_corners[ 2 ] - center, elem_corners[ 3 ] - center,
                                        elem_corners[ 4 ] - center, elem_corners[ 5 ] - center,
    ...
    bool box_tet_overlap( const CartVect* corners_in, const CartVect& center, const CartVect& dims )
    {
        ...
        if( fabs( corners[ 0 ][ 0 ] ) <= dims[ 0 ] && fabs( corners[ 0 ][ 1 ] ) <= dims[ 1 ] &&
            ...
            return true;
        if( fabs( corners[ 1 ][ 0 ] ) <= dims[ 0 ] && fabs( corners[ 1 ][ 1 ] ) <= dims[ 1 ] &&
            fabs( corners[ 1 ][ 2 ] ) <= dims[ 2 ] )
            return true;
        if( fabs( corners[ 2 ][ 0 ] ) <= dims[ 0 ] && fabs( corners[ 2 ][ 1 ] ) <= dims[ 1 ] &&
            fabs( corners[ 2 ][ 2 ] ) <= dims[ 2 ] )
            return true;
        if( fabs( corners[ 3 ][ 0 ] ) <= dims[ 0 ] && fabs( corners[ 3 ][ 1 ] ) <= dims[ 1 ] &&
    ...
    {
        CartVect xi;
        return nat_coords_trilinear_hex( hex, xyz, xi, etol ) && fabs( xi[ 0 ] ) - 1 < etol &&
               fabs( xi[ 1 ] ) - 1 < etol && fabs( xi[ 2 ] ) - 1 < etol;
    }
