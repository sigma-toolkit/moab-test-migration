...
class ReadMCNP5 : public ReaderIface
{
  ...
  private:
    ...
    ErrorCode create_tags( Tag& date_and_time_tag, Tag& title_tag, Tag& nps_tag, Tag& tally_number_tag,
    ...
    ErrorCode average_with_existing_tally( bool debug, unsigned long int& new_nps, unsigned long int nps,
                                           unsigned int tally_number, Tag tally_number_tag, Tag nps_tag, Tag tally_tag,
                                           Tag error_tag, double values[], double errors[], unsigned int n_elements );

    ErrorCode transform_point_to_cartesian( double* in, double* out, coordinate_system coord_sys );

    ErrorCode average_tally_values( const unsigned long int nps0, const unsigned long int nps1, double* values0,
