                               ...
                               const Tag*, int, int )
{
    ...
    MB_CHK_SET_ERR( rval, "Trouble parsing option string" );
    ...
    MB_CHK_SET_ERR( rval, "Trouble processing conventional tags" );
    ...
    MB_CHK_SET_ERR( rval, "Trouble collecting mesh information" );

    rval = myHelper->collect_variable_data( var_names, tstep_nums );
    MB_CHK_SET_ERR( rval, "Trouble collecting variable data" );

    rval = myHelper->init_file( var_names, desired_names, append );
    MB_CHK_SET_ERR( rval, "Trouble initializing file" );

    rval = myHelper->write_values( var_names, tstep_nums );
    MB_CHK_SET_ERR( rval, "Trouble writing values to file" );
...
ErrorCode WriteNC::process_conventional_tags( EntityHandle fileSet )
{
    ...
    MB_CHK_SET_ERR( rval, "Trouble getting conventional tag " << tag_name );
    rval = mbImpl->tag_get_by_ptr( dimNamesTag, &fileSet, 1, &data, &dimNamesSz );
    MB_CHK_SET_ERR( rval, "Trouble getting data of conventional tag " << tag_name );
    ...
    for( std::size_t i = 0; i != static_cast< std::size_t >( varNamesSz ); i++ )
    {
        if( p[i] == '\0' )
        {
            ...
            MB_CHK_SET_ERR( rval, "Trouble getting size of dimensions for variable " << var_name );
            ...
            for( std::size_t j = 0; j != static_cast< std::size_t >( sz ); j++ )
            {
                ...
                MB_CHK_SET_ERR( rval, "Trouble getting dimension of variable " << var_name );
            ...
            Tag varAttTag = 0;
            rval =
                mbImpl->tag_get_handle( tag_name.c_str(), 0, MB_TYPE_OPAQUE, varAttTag, MB_TAG_SPARSE | MB_TAG_VARLEN );
            MB_CHK_SET_ERR( rval, "Trouble getting conventional tag " << tag_name );
            ...
            else
            {
                ...
                MB_CHK_SET_ERR( rval, "Trouble getting length of conventional tag " << tag_name );
                ...
                MB_CHK_SET_ERR( rval, "Trouble processing attributes of variable " << var_name );
    ...
    MB_CHK_SET_ERR( rval, "Trouble processing global attributes" );
