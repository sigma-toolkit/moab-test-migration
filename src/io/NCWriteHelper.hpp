...
class NCWriteHelper
{
  public:
    ...
    static NCWriteHelper* get_nc_helper( WriteNC* writeNC, int fileId, const FileOptions& opts,
    ...
    ErrorCode init_file( std::vector< std::string >& var_names,
                         std::vector< std::string >& desired_names, bool _append );
  ...
  private:
    ...
    ErrorCode write_set_variables( std::vector< WriteNC::VarData >& vsetdatas,
                                   std::vector< int >&              tstep_nums );
            ...
            for( std::size_t k = 0; k != nk; k++ )
                for( std::size_t j = 0; j != nj; j++ )
                    for( std::size_t i = 0; i != size_range; i++ )
                        dest[ idxInSource + k * nij + j * size_range + i ] =
                            source[ idxInSource + j * nik + i * nk + k ];
