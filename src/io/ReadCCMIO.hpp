...
class ReadCCMIO : public ReaderIface
{
  ...
  private:
    ErrorCode read_processor( CCMIOID rootID, CCMIOID problemID, CCMIOID processorID,
                              CCMIOID verticesID, CCMIOID topologyID, CCMIOSize_t proc,
                              Range* new_ents );

    ErrorCode read_cells( CCMIOSize_t proc, CCMIOID processorID, CCMIOID verticesID,
                          CCMIOID topologyID, TupleList& vert_map, Range* new_cells );
    ...
    ErrorCode read_gids_and_types( CCMIOID problemID, CCMIOID topologyID,
                                   std::vector< EntityHandle >& cells );

    ErrorCode read_all_faces( CCMIOID topologyID, TupleList& vert_map, TupleList& face_map
#ifndef READCCMIO_USE_TUPLE_LIST
                              ,
                              SenseList& sense_map
#endif
    ...
    ErrorCode read_faces( CCMIOID faceID, CCMIOEntity bdy_or_int, TupleList& vert_map,
                          TupleList& face_map
#ifndef READCCMIO_USE_TUPLE_LIST
                          ,
                          SenseList& sense_map
    ...
    ErrorCode make_faces( int* farray, TupleList& vert_map, std::vector< EntityHandle >& new_faces,
                          int num_faces );
    ...
    ErrorCode get_processors( CCMIOID stateID, CCMIOID& processorID, CCMIOID& verticesID,
    ...
    ErrorCode load_metadata( CCMIOID rootID, CCMIOID problemID, CCMIOID stateID,
    ...
    ErrorCode get_str_option( const char* opt_str, EntityHandle seth, Tag& tag, CCMIOID node,
    ...
    Tag mRadiationTag, mPorosityIdTag, mSpinIdTag, mGroupIdTag, mColorIdxTag, mProcessorIdTag,
        mLightMaterialTag, mFreeSurfaceMaterialTag;
