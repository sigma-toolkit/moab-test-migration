...
class ReadCCMIO : public ReaderIface
{
  ...
  private:
    ErrorCode read_processor( CCMIOID rootID, CCMIOID problemID, CCMIOID processorID, CCMIOID verticesID,
                              CCMIOID topologyID, CCMIOSize_t proc, Range* new_ents );

    ErrorCode read_cells( CCMIOSize_t proc, CCMIOID processorID, CCMIOID verticesID, CCMIOID topologyID,
    ...
    ErrorCode create_cell_from_faces( std::vector< EntityHandle >& facehs, std::vector< int >& senses,
    ...
    ErrorCode read_faces( CCMIOID faceID, CCMIOEntity bdy_or_int, TupleList& vert_map, TupleList& face_map
#ifndef READCCMIO_USE_TUPLE_LIST
                          ,
                          SenseList& sense_map
#endif
    ...
    ErrorCode read_vertices( CCMIOSize_t proc, CCMIOID processorID, CCMIOID verticesID, CCMIOID topologyID,
                             Range* verts, TupleList& vert_map );

    ErrorCode get_processors( CCMIOID stateID, CCMIOID& processorID, CCMIOID& verticesID, CCMIOID& topologyID,
    ...
    ErrorCode load_metadata( CCMIOID rootID, CCMIOID problemID, CCMIOID stateID, CCMIOID processorID,
