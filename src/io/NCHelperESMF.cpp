...
ErrorCode NCHelperESMF::redistribute_local_cells( int start_cell_idx, ParallelComm* pco )
{
    ...
    if( ScdParData::RCBZOLTAN == _readNC->partMethod )
    {
        ...
        ErrorCode rval             = mbZTool->repartition( xverts, yverts, zverts,
                start_cell_idx, "RCB", localGidCells );MB_CHK_SET_ERR( rval, "Error in Zoltan partitioning" );
        delete mbZTool;
                                                   ...
                                                   Range& faces )
{
    ...
    ErrorCode rval =
        _readNC->readMeshIface->get_element_connect( nLocalCells, maxEdgesPerCell, MBPOLYGON, 0, start_element,
                                                     conn_arr_local_cells, nLocalCells );MB_CHK_SET_ERR( rval, "Failed to create local cells" );
