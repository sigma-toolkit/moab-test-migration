...
ErrorCode TempestRemapper::ComputeOverlapMesh( bool kdtree_search, bool use_tempest )
{
    ...
    if( use_tempest )
    {
        // first, let us ensure we have a valid source and target mesh objects
        // if not, convert the source and target meshes to TempestRemap format
        if( m_covering_source == nullptr )
        {
            // Convert the covering mesh to TempestRemap format
            MB_CHK_SET_ERR( this->ConvertMeshToTempest( moab::Remapper::CoveringMesh ),
                            "Can't convert source coverage mesh to TempestRemap" );
        }
        if( m_target == nullptr )
        {
            // Convert the covering mesh to TempestRemap format
            MB_CHK_SET_ERR( this->ConvertMeshToTempest( moab::Remapper::TargetMesh ),
                            "Can't convert target mesh to TempestRemap" );
        }
        ...
        MB_CHK_SET_ERR( ConvertTempestMesh( moab::Remapper::OverlapMesh ),
                        "Can't convert overlap TempestRemap mesh to MOAB format" );
    }
    else
    {
        ...
        MB_CHK_ERR( this->ConvertOverlapMeshSourceOrdered() );
