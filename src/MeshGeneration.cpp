...
ErrorCode MeshGeneration::BrickInstance( MeshGeneration::BrickOpts& opts )
{
    ...
    if( !parmerge )
    {
        MB_CHK_SET_ERR( mb->tag_get_handle( "HANDLEID", sizeof( long ), MB_TYPE_OPAQUE, new_id_tag,
                                            MB_TAG_CREAT | MB_TAG_DENSE ),
                        "Can't get handle id tag" );
    }
    Tag part_tag;
    int dum_id = -1;
    MB_CHK_SET_ERR( mb->tag_get_handle( "PARALLEL_PARTITION", 1, MB_TYPE_INTEGER, part_tag,
                                        MB_TAG_CREAT | MB_TAG_SPARSE, &dum_id ),
                    "Can't get parallel partition tag" );
    ...
    for( int a = 0; a < A; a++ )
    {
        for( int b = 0; b < B; b++ )
        {
            for( int c = 0; c < C; c++ )
            {
                ...
                MB_CHK_SET_ERR( mb->tag_set_data( global_id_tag, verts, &gids[0] ),
                                "Can't set global ids to vertices" );
                if( !parmerge )
                {
                    MB_CHK_SET_ERR( mb->tag_set_data( new_id_tag, verts, &lgids[0] ),
                                    "Can't set the new handle id tags" );
                }
                ...
                {
                    num_v_per_elem = 27;
                    MB_CHK_SET_ERR( iface->get_element_connect( num_el, 27, MBHEX, 0, starte, conn ),
                                    "Can't get element connectivity" );
                }
                else if( tetra )
                {
                    num_v_per_elem = 4;
                    MB_CHK_SET_ERR( iface->get_element_connect( num_el, 4, MBTET, 0, starte, conn ),
                                    "Can't get element connectivity" );
                }
                else
                {
                    MB_CHK_SET_ERR( iface->get_element_connect( num_el, 8, MBHEX, 0, starte, conn ),
                                    "Can't get element connectivity" );
                }
                ...
                MB_CHK_SET_ERR( iface->update_adjacencies( starte, num_el, num_v_per_elem, conn ),
                ...
                MB_CHK_SET_ERR( mb->tag_set_data( global_id_tag, cells, &gids[0] ),
                                "Can't set global ids to elements" );
                if( !parmerge )
                {
