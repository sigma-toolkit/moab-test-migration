...
ErrorCode MeshGeneration::BrickInstance( MeshGeneration::BrickOpts& opts )
{
    ...
    Tag global_id_tag;
    MB_CHK_SET_ERR( mb->tag_get_handle( "GLOBAL_ID", 1, MB_TYPE_INTEGER, global_id_tag ), "Can't get global id tag"  );

    // set global ids
    Tag new_id_tag;
    if( !parmerge )
    {
        MB_CHK_SET_ERR( mb->tag_get_handle( "HANDLEID", sizeof( long ), MB_TYPE_OPAQUE, new_id_tag, MB_TAG_CREAT | MB_TAG_DENSE ), "Can't get handle id tag"  );
    }
    Tag part_tag;
    int dum_id = -1;
    MB_CHK_SET_ERR( mb->tag_get_handle( "PARALLEL_PARTITION", 1, MB_TYPE_INTEGER, part_tag, MB_TAG_CREAT | MB_TAG_SPARSE, &dum_id ), "Can't get parallel partition tag"  );
    ...
    for( int a = 0; a < A; a++ )
    {
        for( int b = 0; b < B; b++ )
        {
            for( int c = 0; c < C; c++ )
            {
                ...
                EntityHandle startv;
                MB_CHK_SET_ERR( iface->get_node_coords( 3, num_nodes, 0, startv, arrays ), "Can't get node coords"  );
                ...
                MB_CHK_SET_ERR( mb->tag_set_data( global_id_tag, verts, &gids[0] ), "Can't set global ids to vertices"  );
                if( !parmerge )
                {
                    MB_CHK_SET_ERR( mb->tag_set_data( new_id_tag, verts, &lgids[0] ), "Can't set the new handle id tags"  );
                }
                ...
                {
                    num_v_per_elem = 27;
                    MB_CHK_SET_ERR( iface->get_element_connect( num_el, 27, MBHEX, 0, starte, conn ), "Can't get element connectivity"  );
                }
                else if( tetra )
                {
                    num_v_per_elem = 4;
                    MB_CHK_SET_ERR( iface->get_element_connect( num_el, 4, MBTET, 0, starte, conn ), "Can't get element connectivity"  );
                }
                else
                {
                    MB_CHK_SET_ERR( iface->get_element_connect( num_el, 8, MBHEX, 0, starte, conn ), "Can't get element connectivity"  );
                }
                ...
                MB_CHK_SET_ERR( mb->create_meshset( MESHSET_SET, part_set ), "Can't create mesh set"  );
                MB_CHK_SET_ERR( mb->add_entities( part_set, cells ), "Can't add entities to set"  );
                all3dcells.merge( cells );
                // update adjacencies now, because some elements are new;
                MB_CHK_SET_ERR( iface->update_adjacencies( starte, num_el, num_v_per_elem, conn ), "Can't update adjacencies"  );
                // If needed, add all edges and faces
                if( adjEnts )
                {
                    // Generate all adj entities dimension 1 and 2 (edges and faces/ tri or qua)
                    Range edges, faces;
                    MB_CHK_SET_ERR( mb->get_adjacencies( cells, 1, true, edges, Interface::UNION ), "Can't get edges"  );
                    MB_CHK_SET_ERR( mb->get_adjacencies( cells, 2, true, faces, Interface::UNION ), "Can't get faces"  );
                    // MB_CHK_SET_ERR( mb->add_entities(part_set, edges), "Can't add
                    // edges to partition set" ); MB_CHK_SET_ERR( mb->add_entities(part_set,
                    // faces), "Can't add faces to partition set" );
                }

                MB_CHK_SET_ERR( mb->tag_set_data( global_id_tag, cells, &gids[0] ), "Can't set global ids to elements"  );
                if( !parmerge )
                {
                    MB_CHK_SET_ERR( mb->tag_set_data( new_id_tag, cells, &lgids[0] ), "Can't set new ids to elements"  );
                }
                int part_num = a + m * A + ( b + n * B ) * ( M * A ) + ( c + k * C ) * ( M * A * N * B );
                MB_CHK_SET_ERR( mb->tag_set_data( part_tag, &part_set, 1, &part_num ), "Can't set part tag on set"  );
    ...
    MB_CHK_SET_ERR( mb->add_entities( cset, wsets ), "Can't add entity sets"  );
#ifdef MOAB_HAVE_MPI
    ...
    MB_CHK_SET_ERR(mb->write_file("test0.h5m", 0, ";;PARALLEL=WRITE_PART"), "Can't write
    in parallel, before merging");
    */
    // After the mesh is generated on each proc, merge the vertices
    MergeMesh mm( mb );

    // MB_CHK_SET_ERR( mb->get_entities_by_dimension(0, 3, all3dcells), "Can't get all 3d
    // cells elements" );
    ...
    {  // Merge needed
        if( newMergeMethod )
        {
            MB_CHK_SET_ERR( mm.merge_using_integer_tag( localVerts, global_id_tag ), "Can't merge"  );
        }
        else
        {
            MB_CHK_SET_ERR( mm.merge_entities( all3dcells, 0.0001 ), "Can't merge"  );
        }
#ifndef _MSC_VER /* windows */
        if( 0 == rank )
        {
    ...
    if( adjEnts )
    {
        for( Range::iterator wsit = wsets.begin(); wsit != wsets.end(); ++wsit )
        {
            ...
            MB_CHK_SET_ERR( mb->get_entities_by_dimension( ws, 3, cells ), "Can't get cells"  );
            MB_CHK_SET_ERR( mb->get_adjacencies( cells, 1, false, edges, Interface::UNION ), "Can't get edges"  );
            MB_CHK_SET_ERR( mb->get_adjacencies( cells, 2, false, faces, Interface::UNION ), "Can't get faces"  );
            MB_CHK_SET_ERR( mb->add_entities( ws, edges ), "Can't add edges to partition set"  );
            MB_CHK_SET_ERR( mb->add_entities( ws, faces ), "Can't add faces to partition set"  );
        }
    }
#ifdef MOAB_HAVE_MPI
    if( size > 1 )
    {
        ...
        // MB_CHK_SET_ERR( mb->create_meshset(MESHSET_SET, mesh_set), "Can't create new
        // set" );

        if( parmerge )
        {
            ParallelMergeMesh pm( pc, 0.00001 );
            MB_CHK_SET_ERR( pm.merge(), "Can't resolve shared ents"  );
#ifndef _MSC_VER /* windows */
            if( 0 == rank )
            {
        ...
        {
            MB_CHK_SET_ERR( pc->resolve_shared_ents( cset, -1, -1, &new_id_tag ), "Can't resolve shared ents"  );
#ifndef _MSC_VER /* windows */
            if( 0 == rank )
            {
        ...
        {  // Default is to delete the 1- and 2-dimensional entities
            ...
            Range toDelete;
            MB_CHK_SET_ERR( mb->get_entities_by_dimension( cset, 1, toDelete ), "Can't get edges"  );

            MB_CHK_SET_ERR( mb->get_entities_by_dimension( cset, 2, toDelete ), "Can't get faces"  );

            MB_CHK_SET_ERR( pc->delete_entities( toDelete ), "Can't delete entities"  );
            MB_CHK_SET_ERR( mb->remove_entities( cset, toDelete ), "Can't remove entities from base set"  );
            if( 0 == rank )
            {
        ...
        if( GL > 0 )
        {
            MB_CHK_ERR( pc->exchange_ghost_cells( 3,   // int ghost_dim
                                             0,   // int bridge_dim
                                             GL,  // int num_layers
                                             0,   // int addl_ents
    ...
    if( !parmerge )
    {
        MB_CHK_SET_ERR( mb->tag_delete( new_id_tag ), "Can't delete new ID tag"  );
    }
