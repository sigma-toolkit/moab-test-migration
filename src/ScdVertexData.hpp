...
class ScdVertexData : public SequenceData
{
  ...
  public:
    //! constructor
    ScdVertexData( const EntityHandle start_vertex, const int imin, const int jmin, const int kmin,
...
{
    return get_vertex( coords.hom_coord( )[ 0 ], coords.hom_coord( )[ 1 ],
                       coords.hom_coord( )[ 2 ] );
}

inline ErrorCode ScdVertexData::get_params( const EntityHandle vhandle, int& i, int& j,
                                            int& k ) const
{
    ...
    return ( vhandle >= start_handle( ) && i >= i_min( ) && i <= i_max( ) && j >= j_min( ) &&
             j <= j_max( ) && k >= k_min( ) && k <= k_max( ) )
               ? MB_SUCCESS
               : MB_FAILURE;
}
